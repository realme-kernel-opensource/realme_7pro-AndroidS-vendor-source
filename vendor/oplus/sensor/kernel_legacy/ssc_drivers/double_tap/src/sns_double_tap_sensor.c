...
static void double_tap_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "double_tap";
...
static void double_tap_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    DOUBLE_TAP_LOGI("double_tap_publish_available");
...
{
    sns_double_tap_sensor_state *state = (sns_double_tap_sensor_state *)this->state->state;
    ...
    sns_suid_lookup_add(this, &state->suid_lookup_data, "double_tap");
    sns_suid_lookup_add(this, &state->suid_lookup_data, "gravity");

    double_tap_publish_attributes(this);
...
{
    UNUSED_VAR(this);
    DOUBLE_TAP_LOGI("sns_double_tap_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_sensor_instance *sns_double_tap_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    DOUBLE_TAP_LOGI("double_tap_set_client_request: exist_request = 0x%x, new_request = 0x%x", exist_request,
        new_request);
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        DOUBLE_TAP_LOGI("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            DOUBLE_TAP_LOGI("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_double_tap_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            DOUBLE_TAP_LOGI("set_client_config");
    ...
    if (NULL != ret_inst
        && NULL == ret_inst->cb->get_client_request(ret_inst, this->sensor_api->get_sensor_uid(this),
            true)) {
        DOUBLE_TAP_LOGI("remove_instance");
