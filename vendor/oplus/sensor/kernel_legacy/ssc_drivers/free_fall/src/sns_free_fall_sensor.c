...
static void free_fall_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "free_fall";
...
static void free_fall_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    FREE_FALL_LOG_0("free_fall_publish_available");
...
{
    sns_free_fall_sensor_state *state = (sns_free_fall_sensor_state *)this->state->state;
    ...
    free_fall_publish_attributes(this);
    FREE_FALL_LOG_0("free_fall init success and attributes published");
...
{
    UNUSED_VAR(this);
    FREE_FALL_LOG_0("sns_free_fall_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_sensor_instance *sns_free_fall_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    FREE_FALL_LOG_2("free_fall_set_client_request: exist_request = 0x%x, new_request = 0x%x",
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        FREE_FALL_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            FREE_FALL_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_free_fall_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            FREE_FALL_LOG_0("set_client_config");
    ...
    if (NULL != ret_inst
        && NULL == ret_inst->cb->get_client_request(ret_inst, this->sensor_api->get_sensor_uid(this),
            true)) {
        FREE_FALL_LOG_0("remove_instance");
