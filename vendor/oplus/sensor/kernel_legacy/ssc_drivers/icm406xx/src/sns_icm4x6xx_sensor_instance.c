...
static void icm4x6xx_inst_cleanup(sns_sensor_instance *const this)
{
  ...
  icm4x6xx_en_pedometer(this, false);
#endif
                        ...
                        sns_sensor_state const *sstate)
{
    ...
    state->rigid_body_type = sensor_state->common.registry_pf_cfg.rigid_body_type;
    ICM4X6XX_INST_PRINTF(LOW, this, "icm4x6xx_inst_init hw_idx=%d, reg_idx=%d, inst_idx=%d, rigid=%d",
            (uint32_t)sensor_state->hardware_id, sensor_state->registration_idx,
            state->instance_idx, state->rigid_body_type);
    #else
    ...
    ICM4X6XX_INST_PRINTF(MED, this, "invn product %d, whoami 0x%x, hi_res %d, hi_range %d",
                state->product_info.series,
                state->product_info.whoami,
    ...
    ICM4X6XX_INST_PRINTF(MED, this, "a/g res_idx: %d %d, a/g fsr: %d %d, a/g res*1M:%d %d",
                sensor_state->common.a_res_idx, sensor_state->common.g_res_idx,
                state->accel_info.fsr, state->gyro_info.fsr,
                (uint32_t)(ACC_RES_M2_PER_S(state->accel_info.fsr, state->product_info.use_hi_res,
                                             state->product_info.hi_range) * 1000 * 1000),
                (uint32_t)(GYRO_RES_RPS(state->gyro_info.fsr, state->product_info.use_hi_res,
                                             state->product_info.hi_range) * 1000 * 1000));
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    sns_memscpy(&state->step_counter_info.suid,
            sizeof(state->step_counter_info.suid),
            &sensor_state->common.step_counter_suid,
            sizeof(state->step_counter_info.suid));
#endif
    ...
    state->gyro_info.last_off_tick = 0;
#endif
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    state->step_counter_info.desire_en = false;
    state->step_counter_info.timer_data_stream = NULL;
#endif
...
#ifndef ICM4X6XX_DISABLE_DAE
if(sensor_state->common.dae_if.ag.state)
{
    ICM4X6XX_INST_PRINTF(HIGH, this, "dae_valid(1): disable flushing_fifo");
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    state->encoded_sensor_pedometer_event_len = pb_get_encoded_size_sensor_stream_event(step_counter_data, 1);
#endif
    ...
    state->accel_registry_cfg.fac_cal_corr_mat.e00 = 1.0f;
    state->accel_registry_cfg.fac_cal_corr_mat.e11 = 1.0f;
    state->accel_registry_cfg.fac_cal_corr_mat.e22 = 1.0f;
    ...
    if(sensor_state->common.dae_if.ag.state)
	{
        ICM4X6XX_INST_PRINTF(HIGH, this, "dae_valid(1): init dae if");
		icm4x6xx_dae_if_init(this, stream_mgr, &dae_suid, sensor_state);
	}
	else
#endif
	{
		ICM4X6XX_INST_PRINTF(LOW, this, "dae_valid(0): register interrupt");
...
int icm4x6xx_self_test_gyro_response(sns_sensor_instance *instance)
{
    ...
    if(sns_convert_ns_to_ticks(GYRO_RING_DOWN_TIME) > delta) {
        ICM4X6XX_INST_PRINTF(MED, instance, "gyro postpone %x%08x",
                (uint32_t)(delta >> 32), (uint32_t)(delta));
        rc += sns_busy_wait(sns_convert_ns_to_ticks(GYRO_RING_DOWN_TIME) - delta);
    }
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (st_otp[i] != 0) {
            ...
            if ((ratio <= 0.5f) || ((ratio >= 1.5f) && (state->product_info.series == YOKO_C1))) {
                ICM4X6XX_INST_PRINTF(HIGH, instance, "gyro st test fail0");
                rc += SNS_RC_FAILED;
            }
        } else {
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    state->step_counter_info.is_enabled = state->st_resume.en_pedometer;
    state->step_counter_info.desire_en = state->st_resume.desire_en_pedometer;
    state->step_counter_info.desire_publish = state->st_resume.desire_pub_pedometer;
#endif
...
void icm4x6xx_start_factory_test(sns_sensor_instance* instance, icm4x6xx_sensor_type sensor)
{
  ...
  if(sensor == ICM4X6XX_ACCEL)
  {
    ...
    state->fac_test_info.variance_threshold = 0.060025f; // (25mg*G)^2
  ...
  else if(sensor == ICM4X6XX_GYRO)
  {
    ...
    state->fac_test_info.variance_threshold = 25;//0.000438f; // (1.2dps*PI/180)^2
  ...
  if (icm4x6xx_dae_if_available(instance) &&
     (ICM4X6XX_CONFIG_IDLE == state->config_step) &&
...
void icm4x6xx_run_self_test(sns_sensor_instance *instance)
{
    ...
    if(state->accel_info.test_info.test_client_present) {
        ...
        if (state->accel_info.test_info.test_type == SNS_PHYSICAL_SENSOR_TEST_TYPE_COM) {
            ...
            #ifdef ICM4X6XX_ENABLE_PEDOMETER
            sns_sensor_util_remove_sensor_instance_stream(instance, &state->step_counter_info.timer_data_stream);
            #endif
            #ifdef ICM4X6XX_GYRO_RING_DOWN_WA
            sns_sensor_util_remove_sensor_instance_stream(instance, &state->gyro_info.timer_postpone_stream);
            #endif
            icm4x6xx_st_store_pre_state(instance);
#ifndef ICM4X6XX_DISABLE_DAE
            if(icm4x6xx_dae_if_available(instance)) {
                icm4x6xx_dae_if_stop_ag_streaming(instance);
                icm4x6xx_dae_if_stop_temp_streaming(instance);
            }
#endif
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    if(state->step_counter_info.test_info.test_client_present) {
        ICM4X6XX_INST_PRINTF(MED, instance, "self-test test_type = %d", state->step_counter_info.test_info.test_type);
        if(state->step_counter_info.test_info.test_type == SNS_PHYSICAL_SENSOR_TEST_TYPE_FACTORY) {
            if (who_am_i_success) {
                rc = 0;
            icm4x6xx_send_self_test_event(instance, &state->step_counter_info.suid,
                                !rc, SNS_PHYSICAL_SENSOR_TEST_TYPE_FACTORY, rc);

                ICM4X6XX_INST_PRINTF(HIGH, instance, "step count calibrate success" );
            }
        } else {
            ICM4X6XX_INST_PRINTF(HIGH, instance, "unsupported test type for this sensor");
        }
        state->step_counter_info.test_info.test_client_present = false;
    }
#endif
...
void icm4x6xx_process_fac_test(sns_sensor_instance *instance)
{
  ...
  {
    ICM4X6XX_INST_PRINTF(MED, instance, "Sample Sums: %d/1M %d/1M %d/1M",
                    (int32_t)(state->fac_test_info.sample_sum[0]*1000000),
                    (int32_t)(state->fac_test_info.sample_sum[1]*1000000),
                    (int32_t)(state->fac_test_info.sample_sum[2]*1000000));
    ICM4X6XX_INST_PRINTF(MED, instance, "Sample square sums: %d/1M %d/1M %d/1M",
                    (int32_t)(state->fac_test_info.sample_square_sum[0]*1000000),
                    (int32_t)(state->fac_test_info.sample_square_sum[1]*1000000),
                    (int32_t)(state->fac_test_info.sample_square_sum[2]*1000000));
    ...
    ICM4X6XX_VERB_INST_PRINTF(MED, instance, "fac_cal_bias: %d/1M %d/1M %d/1M",
                    (int32_t)(fac_cal_bias[0]*1000000),
                    (int32_t)(fac_cal_bias[1]*1000000),
                    (int32_t)(fac_cal_bias[2]*1000000));
    ...
    if (test_pass)
    {
      ...
      ICM4X6XX_INST_PRINTF(MED, instance, "(rigid=%d)(%#X)fac cal pass, new bias*1k(ver:%d): %d, %d, %d",
                    state->rigid_body_type,
