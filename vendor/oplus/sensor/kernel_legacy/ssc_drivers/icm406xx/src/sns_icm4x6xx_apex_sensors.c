...
#if defined ICM4X6XX_ENABLE_LOWG || defined ICM4X6XX_ENABLE_HIGHG || defined ICM4X6XX_ENABLE_PEDOMETER
static void
...
sns_rc icm4x6xx_free_fall_init(sns_sensor *const this)
{
  ...
  /* QCOM: reverse the order of these two functions, so that icm4x6xx_publish_attributes
   * is called last, to avoid the last attribute not being properly sent.
   */
 ...
 */
static void
icm4x6xx_publish_step_counter_attributes(sns_sensor *const this)
{
  icm4x6xx_state *state = (icm4x6xx_state*)this->state->state;
  {
    sns_std_attr_value_data values[] = {SNS_ATTR, SNS_ATTR};
    values[0].has_flt = true;
    values[0].flt = ICM4X6XX_STEP_COUNTER_ODR_1;
    values[1].has_flt = true;
    values[1].flt = ICM4X6XX_STEP_COUNTER_ODR_5;
...
{
  icm4x6xx_state *state = (icm4x6xx_state*)this->state->state;
  sns_sensor_uid suid = (sns_sensor_uid)ICM_STEP_COUNTER_SUID;
  ...
  icm4x6xx_common_init(this);
  icm4x6xx_publish_step_counter_attributes(this);
  icm4x6xx_publish_attributes(this);
