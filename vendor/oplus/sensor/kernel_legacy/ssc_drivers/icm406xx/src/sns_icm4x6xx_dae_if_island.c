  ...
  sns_dae_set_static_config  *config_req)
{
    ...
    if ((YOKOHAMA == sensor_state->common.product_info.series) ||
        (YOKO_C1 == sensor_state->common.product_info.series)) {
      dae_if->ag.nano_hal_vtable_name =
        (sensor_state->hardware_id == 0) ? "icm4x6xx_hal_table" : "icm4x6xx_hal_table2";
    ...
    config_req->accel_info.accel_attr[0].value.sint = sensor_state->common.registry_pf_cfg.rigid_body_type;
  ...
  sns_dae_set_static_config *config_req)
{
    ...
    if ((YOKOHAMA == sensor_state->common.product_info.series) ||
        ...
        dae_if->temp.nano_hal_vtable_name =
            (sensor_state->hardware_id == 0) ? "icm4x6xx_temperature_hal_table" : "icm4x6xx_temperature_hal_table2";
    } else if (HAVANA == sensor_state->common.product_info.series) {
        dae_if->temp.nano_hal_vtable_name =
            ...
            icm4x6xx_instance_state *state)
{
  ...
  if (state->fifo_info.cur_wmk > 0) {
      ...
      if (config_req.data_age_limit_ticks < (uint64_t)((double)UINT64_MAX * 0.9f)) {
        config_req.data_age_limit_ticks += config_req.data_age_limit_ticks / 10;
      }
  } else {  //MD only
      config_req.dae_watermark = 0;
  ...
  ICM4X6XX_INST_PRINTF(HIGH, this, "dae_wm=%u, dae_flush_period=0x%x%08x, wm=%u, expected %d B",
                     config_req.dae_watermark, (uint32_t)(config_req.data_age_limit_ticks>>32),
  ...
  sns_sensor_instance      *this)
{
  ...
  if (config_req.data_age_limit_ticks < (uint64_t)((double)UINT64_MAX * 0.9)) {
    config_req.data_age_limit_ticks += config_req.data_age_limit_ticks / 10;
  }
...
/* ------------------------------------------------------------------------------------ */
/* Stop DAE from collecting data from sensor FIFO when interrupt occurs */
            ...
            pb_istream_t        *pbstream)
{
    ...
    if(pb_decode(pbstream, sns_dae_resp_fields, &resp))
    {
        ...
        case SNS_DAE_MSGID_SNS_DAE_FLUSH_HW:
            ...
            if (dae_stream == &state->dae_if.ag) {
                /**
                 * When ODR change, or new request comes, Then below sequence can be observed
                 * Driver receives sns_std_sensor_config
                 * Driver sends sns_dae_flush_hw to DAE
                 * Driver sends existing sns_std_sensor_physical_config_event
                 * Driver sends sns_dae_pause_sampling to DAE sensor
                 * Driver sends sns_dae_flush_data_events to DAE sensor
                 * Driver sends new sns_dae_set_streaming_config to DAE sensor
                 * Driver sends new sns_std_sensor_physical_config_event
                 */
                if (icm4x6xx_need_to_reset_fifo(state) ||
                    (state->a_new_stream && !icm4x6xx_need_to_reset_fifo(state))) {
                ...
                if(ICM4X6XX_CONFIG_FLUSHING_HW == state->config_step) {
                    ...
                    if (icm4x6xx_need_to_reset_fifo(state)) {
                        ...
                        ICM4X6XX_INST_PRINTF(LOW, this, "set_fifo_mode BYPASS");
                    }
                    //icm4x6xx_reconfig_hw(this);
                } else {
                    if ((state->accel_info.desire_publish ||
    ...
    case SNS_DAE_MSGID_SNS_DAE_PAUSE_SAMPLING:
      ...
      #ifdef ICM4X6XX_DAE_MISSING_604_WA
      /* WA for no 604 response from DAE after driver send 603, 605 and 604 */
      if (NULL != state->dae_if.ag.stream &&
          dae_stream->state != STREAM_STOPPING) {
          if(ICM4X6XX_CONFIG_STOPPING_STREAM == state->config_step) {
              ICM4X6XX_INST_PRINTF(HIGH, this, "wa for 604");
              icm4x6xx_reconfig_hw(this);
          }
      }
      #endif
...
bool icm4x6xx_dae_if_available(sns_sensor_instance *this)
{
    ...
    bool dae_valid = (stream_usable(&dae_if->ag) && stream_usable(&dae_if->temp));
    ICM4X6XX_VERB_INST_PRINTF(LOW, this,  "dae_valid(%d)", dae_valid);
  return dae_valid;
}
    ...
    icm4x6xx_state          *sensor_state)
{
    ...
    ICM4X6XX_VERB_INST_PRINTF(LOW, this, "icm4x6xx_dae_if_init ag.state=%d temp.state=%d",
...
/* ------------------------------------------------------------------------------------ */
bool icm4x6xx_dae_if_stop_ag_streaming(sns_sensor_instance *this)
{
    bool cmd_sent = false;
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    icm4x6xx_dae_if_info    *dae_if = &state->dae_if;
  ...
  if(stream_usable(&state->dae_if.ag) &&
    (dae_if->ag.state == STREAMING || dae_if->ag.state == STREAM_STARTING))
    {
        ...
        #ifdef ICM4X6XX_DAE_MISSING_604_WA
        if (true == cmd_sent) {
            state->config_step = ICM4X6XX_CONFIG_STOPPING_STREAM;
        }
        #endif
    }
...
bool icm4x6xx_dae_if_start_ag_streaming(sns_sensor_instance *this)
{
    bool cmd_sent = false;
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    icm4x6xx_dae_if_info    *dae_if = &state->dae_if;
...
#else /*#define ICM4X6XX_DISABLE_DAE*/
...
#endif /*#ifndef ICM4X6XX_DISABLE_DAE*/
