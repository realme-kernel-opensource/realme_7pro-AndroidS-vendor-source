...
#define ICM4X6XX_DRIVER_VERSION  0x132
 ...
 * --------     --------   ----------       ---------------------------------
 * 03/16/20     0x0132     TDK              Add support for Havana LPA
 * 03/11/20     0x0132     TDK              add new whoami for icm42631 which belong to Yoko C1
 * 03/04/20     0x0132     TDK              force soft reset sensor when bus communicate fail
 * 03/02/20     0x0132     TDK              Add support for Yoko-C1 icm42632-m
 * 02/28/20     0x0132     TDK              read out all interrupt events for removing duplicated message
 * 02/26/20     0x0132     TDK              add new part number ICM40607A support
 * 02/24/20     0x0132     TDK              wrapper icm4x6xx_set_reg_bank function to reduce duplicated code
 * 02/13/20     0x0131     TDK              Enable gyro stall workaround
 * 01/24/20     0x0131     TDK              Added variable to save the previous max_requested_flush_ticks
 * 01/23/20     0x0131     TDK              Force set fake a/g samples as unreliable
 * 01/20/20     0x0131     TDK              disable 604 workaround
 * 01/17/20     0x0131     TDK              Fix CTS 10.0 R2 gyro stddev failures
 * 01/17/20     0x0131     TDK              Optimize gyro ring down workaround
 * 01/17/20     0x0131     TDK              fix data dropping issue in SNS-2639
 * 01/17/20     0x0131     TDK              Add workaround for DAE missing 604 issue
 * 01/13/20     0x0131     TDK              set curr_format to real fifo format when not match
 * 12/23/19     0x0131     TDK              Not force set chosen_flush_period_ticks=max(flush_period, batch_period)
 * 12/21/19     0x0131     TDK              [DAE] combine registers read to be more efficient.
 * 12/19/19     0x0131     TDK              send pause sampling to DAE when ODR changed.
 * 12/18/19     0x0131     TDK              send physical_config_event and MD disabled event
 * 12/16/19     0x0131     TDK              Send DAE WMK 0 when MD enabled, request from QCOM.
 * 12/13/19     0x0131     TDK              Add workaround for fifo format not match issue
 * 12/13/19     0x0131     TDK              fix bug: freefall request be accidently rejected
 * 12/13/19     0x0131     TDK              Fix passive QAA build 78 failures
 * 12/13/19     0x0131     TDK              fix QAA lacking of sns_cal_event failures.
 * 12/03/19     0x0130     TDK              add optimization for DAE nano driver
 * 12/03/19     0x0130     TDK              other small changes
 * 11/21/19     0x0130     TDK              Fixed QAA build 78 adjacent phy config mismatch with ts failures
 * 11/21/19     0x0130     TDK              Support DAE enabled in macro but disabled by registry
 * 11/20/19     0x0130     TDK              fix AMD false triggered issue by only fed acc samples
 * 11/20/19     0x0130     TDK              Handle invalid(on-change) request of A/G/T
 * 11/20/19     0x0130     TDK              Discard more unreliable accel samples
 * 10/30/19     0x0130     TDK              revert ts variables from float to double to fix CTS jitter issue
 * 10/30/19     0x0130     TDK              Add workaround for YOK gyro stall issue
 * 10/30/19     0x0130     TDK              fix duplicated sensor_temperature UUID bug
 * 10/16/19     0x0130     TDK              Optimize code according to QCOM's comments
 * 10/16/19     0x0130     TDK              revert interval calculate algo factor tolerance
 * 10/16/19     0x0130     TDK              send sample rate 0 cfg event for gated event
 * 09/19/19     0x0126     TDK              Disable Accel first before Accel mode change
 * 09/19/19     0x0126     TDK              Fix acceptance v74.1 test failures
 * 09/19/19     0x0126     TDK              Add interrupt level mode suports from QCOM's request
 * 09/19/19     0x0126     TDK              Add passive sensor suports
 * 09/19/19     0x0126     TDK              Add dual sensor suports
 * 09/19/19     0x0126     TDK              Add HW Pedometer suports
