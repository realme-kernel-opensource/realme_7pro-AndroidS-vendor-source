...
{
  icm4x6xx_state *state = (icm4x6xx_state*)this->state->state;
  ICM4X6XX_VERB_PRINTF(MED, this, "s=%d state=%d", state->sensor, pwr_rail_pend_state);
...
* @paramp[i] vote   rail vote
* @paramp[i] rail_on_timestamp   Timestamp in ticks when the
...
* @return none
*/
static void icm4x6xx_update_rail_vote(
    sns_sensor            *this,
    sns_power_rail_state  vote,
    sns_time              *on_timestamp)
{
    icm4x6xx_state *sensor_state = (icm4x6xx_state*)this->state->state;
    ICM4X6XX_VERB_PRINTF(MED, this, "s=%d rail_vote=%d", sensor_state->sensor, sensor_state->common.rail_config.rail_vote);
    if(sensor_state->common.rail_config.rail_vote != vote)
    {
        sensor_state->common.rail_config.rail_vote = vote;
        ...
        while(NULL != event)
        {
            ...
            if(SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT == event->message_id)
            {
                if(pb_decode(&stream, sns_timer_sensor_event_fields, &timer_event))
                {
                    ...
                    else if(state->power_rail_pend_state == ICM4X6XX_POWER_RAIL_PENDING_OFF)
                    {
                        icm4x6xx_update_rail_vote(this, SNS_RAIL_OFF, NULL);
                        state->power_rail_pend_state = ICM4X6XX_POWER_RAIL_PENDING_NONE;
                    }
                }
                else
...
static void icm4x6xx_get_imu_config(sns_sensor *this,
            ...
            float * restrict chosen_sample_rate,
            float * restrict chosen_report_rate,
            float * restrict chosen_dev_wakeup_sdc_rate,
            sns_time * restrict chosen_flush_period_ticks,
            bool * restrict non_gated_sensor_client_present,
            bool * restrict gated_sensor_client_present,
            bool * restrict max_batch,
            bool * restrict flush_only)
{
    icm4x6xx_state *state = (icm4x6xx_state*)this->state->state;
#ifdef ICM4X6XX_PASSIVE_SENSOR_SUPPORT
    icm4x6xx_instance_state *inst_state =
        (icm4x6xx_instance_state*)instance->state->state;
    inst_state->passive_client_present &= ~sensor_type;
#endif
    ...
    *chosen_flush_period_ticks = 0;
           ...
           request->message_id == SNS_STD_EVENT_GATED_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG) {
            if(icm4x6xx_get_decoded_imu_request(this, request, &decoded_request, &decoded_payload)) {
                ...
                *chosen_sample_rate = SNS_MAX(*chosen_sample_rate,
                ...
                ICM4X6XX_PRINTF(MED, this, "(%d)hb %u hfp %d bp %d fp %d sr %d/100",
                                sensor_type,
                ...
                if(decoded_request.has_batching) {
                    ...
                    if (decoded_request.batching.has_flush_period) {
                      /* Not force chosen_flush_period_ticks=max(flush_period, batch_period), there is client send
                       * flush_period 0 but batch_period not 0 request */
                ...
                *chosen_dev_wakeup_sdc_rate = SNS_MAX(*chosen_dev_wakeup_sdc_rate, wakeup_sdc_rate);
                ...
                if (decoded_request.has_batching &&
                    ...
                    report_rate = (1.0f / (double)UINT32_MAX);
                }
    ...
    if (is_max_batch) {
        if (*non_gated_sensor_client_present ||
            ...
            *chosen_report_rate = (1.0f / (double)UINT32_MAX);
        }
    }
...
static void icm4x6xx_get_step_counter_config(
            sns_sensor_instance *instance,
            bool *is_client_present)
{
  icm4x6xx_instance_state *inst_state =
     (icm4x6xx_instance_state*)instance->state->state;
  sns_sensor_uid *suid = &inst_state->step_counter_info.suid;
  sns_request const *request;
  ...
  *is_client_present = false;
      ...
      request = instance->cb->get_client_request(instance, suid, false))
  {
    if ((request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG) ||
        ...
        inst_state->step_counter_info.is_new_req = false;
  ...
  ICM4X6XX_INST_PRINTF(MED, instance, "icm4x6xx_get_step_counter_config--, client_cnt=%d", client_cnt);
  #endif

  inst_state->step_counter_info.client_cnt = client_cnt;
  client_cnt = 0;
}
#endif
...
static void icm4x6xx_get_sensor_temp_config(sns_sensor *this,
            ...
            float * restrict chosen_report_rate,
            sns_time * restrict chosen_flush_period_ticks,
            bool * restrict sensor_temp_client_present,
            bool * restrict max_batch,
            bool * restrict flush_only)
{
    ...
    *chosen_flush_period_ticks = 0;
#ifdef ICM4X6XX_PASSIVE_SENSOR_SUPPORT
    inst_state->passive_client_present &= ~ICM4X6XX_SENSOR_TEMP;
#endif
...
{
    icm4x6xx_state *state = (icm4x6xx_state *)this->state->state;
    ICM4X6XX_PRINTF(MED, this, "turn rails off s=%d rail_vote=%d", state->sensor, state->common.rail_config.rail_vote);
        ...
        sensor = this->cb->get_library_sensor(this, false))
    {
        ...
        ICM4X6XX_VERB_PRINTF(MED, sensor, "s=%d v=%d", sensor_state->sensor, sensor_state->common.rail_config.rail_vote);
        if(sensor_state->common.rail_config.rail_vote != SNS_RAIL_OFF)
        {
            sns_time timeout = sns_convert_ns_to_ticks(ICM4X6XX_POWER_RAIL_OFF_TIMEOUT_NS);
            icm4x6xx_start_power_rail_timer(sensor, timeout, ICM4X6XX_POWER_RAIL_PENDING_OFF);
            return;
        }
    }
    return;
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
  else if(state->sensor == ICM4X6XX_STEP_COUNTER)
  {
    self_test_info = &inst_state->step_counter_info.test_info;
  }
#endif
  else
  {
                              ...
                              icm4x6xx_sensor_type sensor_type)
{
    ...
    ICM4X6XX_PRINTF(MED, this, "A: rr %u/100 sr %u/100 flush_period %u gated client %u",
             ...
             (uint32_t)tick2us(chosen_flush_period_ticks), a_sensor_client_present_gated);
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    bool step_counter_client_present = false;
    icm4x6xx_get_step_counter_config(instance, &step_counter_client_present);

    inst_state->step_counter_info.is_client_present = step_counter_client_present;
#endif
    ...
    ICM4X6XX_PRINTF(MED, this, "T: rr %u sr %u flush_period %u",
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    inst_state->step_counter_info.desire_publish = false;
#endif
    ...
    ICM4X6XX_PRINTF(MED, this, "a ngatd/gatd %d/%d, g client %d, md client/en %d/%d",
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    ICM4X6XX_PRINTF(MED, this, "step_counter: client %d en %d",
                inst_state->step_counter_info.is_client_present,
                inst_state->step_counter_info.desire_en);
#endif
#ifdef ICM4X6XX_PASSIVE_SENSOR_SUPPORT
    ICM4X6XX_PRINTF(MED, this, "passive: client 0x%x",
                inst_state->passive_client_present)
#endif
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    if(step_counter_client_present)
    {
        //need to en acc sensor for step counter to work
        inst_state->accel_info.desire_en = true;
        inst_state->step_counter_info.desire_en = true;
        inst_state->step_counter_info.desire_publish = true;
    }
#endif
...
#ifndef ICM4X6XX_DISABLE_DAE
        (!icm4x6xx_dae_if_available(instance)) &&
#endif
                                                ...
                                                bool remove)
{
    ...
    if(remove)
    {
        if(NULL != instance)
        {
            ...
            if (exist_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG ||
                exist_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG ||
                exist_request->message_id == SNS_STD_EVENT_GATED_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG) {
    ...
    else
    {
        ...
        if(NULL == instance)
        {
            ICM4X6XX_PRINTF(MED, this, "inst not created");
        ...
        else
        {
            ...
            ICM4X6XX_PRINTF(MED, this, "inst created, new req msg_id=%d, hw_idx=%d",
                new_request->message_id, (uint32_t)state->hardware_id);
        ...
        if(NULL != instance)
        {
            ...
            if(NULL != new_request)
            {
                ...
                if (new_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG
...
#ifdef ICM4X6XX_ENABLE_LOWG
                else if (new_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG
                    ...
                    && state->sensor == ICM4X6XX_STEP_COUNTER) {
                    if (inst_state->step_counter_info.desire_en) {
                        reval_config = false;
                        //there is exsisting step_counter client already present, just send event
                        icm4x6xx_send_config_event(instance);
                        ICM4X6XX_PRINTF(LOW,this,"step_counter is in, just send event");
                    } else {
                        inst_state->step_counter_info.is_new_req = true;
                    }
                }
#endif /* ICM4X6XX_ENABLE_PEDOMETER */
                /* QCOM: As accell is streaming sensor and not a 'on change' sensor it should
                 * reject this configuration with error event */
                else if (new_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_ON_CHANGE_CONFIG) {
                    ICM4X6XX_PRINTF(LOW, this, "(0x%x)reject invalid req", state->sensor);
                    icm4x6xx_publish_error(instance, state->sensor, SNS_STD_ERROR_NOT_SUPPORTED);
...
#ifdef ICM4X6XX_W_BIAS_TO_REG_IMMEDIATELY
  {
    ...
    if(inst_state->update_fac_cal_in_registry)
    {
        ...
        if((ICM4X6XX_ACCEL == state->sensor) || (ICM4X6XX_GYRO == state->sensor))
            icm4x6xx_update_registry(this, instance, state->sensor);
        icm4x6xx_update_sensor_state(this, instance);
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
        && NULL == instance->cb->get_client_request(instance,
            &state->common.step_counter_suid, true)
#endif
       )
    {
...
#endif
        ICM4X6XX_PRINTF(MED, this, "(0x%x)no client present, rm inst(0x%x), hw_id=%d",
                state->sensor, instance, (uint32_t)state->hardware_id);
...
{
  .struct_len         = sizeof(sns_sensor_api),
  .init               = &icm4x6xx_step_counter_init,
  .deinit             = &icm4x6xx_step_counter_deinit,
  .get_sensor_uid     = &icm4x6xx_get_sensor_uid,
  .set_client_request = &icm4x6xx_set_client_request,
  .notify_event       = &icm4x6xx_sensor_notify_event,
};
