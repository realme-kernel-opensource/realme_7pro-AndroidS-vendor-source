...
static const icm4x6xx_odr_reg_map inv_reg_map[] = {
    ...
    },
    {
        ...
        .a_discard_samples = 2, // DS: 40ms not enough
        .g_discard_samples = 3, // TODO: find the root cause
    },
    {
        .odr = ICM4X6XX_ODR_50,
        .odr_reg_value = ODR_50HZ,
        .a_discard_samples = 2, // DS: 20ms not enough
        .g_discard_samples = 4,
    },
    {
    ...
    },
    {
        ...
        .a_discard_samples = 4,
        .g_discard_samples = 8, // DS: 5*8=40ms, for solve GYRO ARcore test FAIL
    },
    {
...
void icm4x6xx_send_cal_event(sns_sensor_instance *const instance, icm4x6xx_sensor_type sensor_type)
{
  ...
  ICM4X6XX_INST_PRINTF(HIGH, instance, "[CAL EVENT] sensor_type:[%d]", sensor_type);
  ...
  if(sensor_type == ICM4X6XX_ACCEL) {
    ...
    if((reg_cfg->fac_cal_bias[0] == state->accel_info.last_cal_info.bias[0]) &&
       ...
       (reg_cfg->fac_cal_bias[2] == state->accel_info.last_cal_info.bias[2])) {
        ...
        ICM4X6XX_INST_PRINTF(HIGH, instance, "[ICM4X6XX_ACCEL] The same with last_cal_info.....");
    } else {
        ...
        state->accel_info.last_cal_info.timestamp = cal_event_ts;
        is_new_bias = true;
        ICM4X6XX_INST_PRINTF(HIGH, instance, "[ICM4X6XX_ACCEL] New cal_info.....");
    }
  } else {
    ...
    if((reg_cfg->fac_cal_bias[0] == state->gyro_info.last_cal_info.bias[0]) &&
       ...
       (reg_cfg->fac_cal_bias[2] == state->gyro_info.last_cal_info.bias[2])) {
        ...
        ICM4X6XX_INST_PRINTF(HIGH, instance, "[ICM4X6XX_GRYO] The same with last_cal_info.....");
    } else {
        sns_memscpy(state->gyro_info.last_cal_info.bias, sizeof(state->gyro_info.last_cal_info.bias),
                reg_cfg->fac_cal_bias, sizeof(reg_cfg->fac_cal_bias));
        state->gyro_info.last_cal_info.timestamp = cal_event_ts;
        is_new_bias = true;
        ICM4X6XX_INST_PRINTF(HIGH, instance, "[ICM4X6XX_GRYO] New cal_info.....");
    }
  }
...
void icm4x6xx_send_config_event(sns_sensor_instance *const instance)
{
    ...
    /* every sensor must use it's own temporary ts */
    ...
    /* If no sensors are enabled for streaming or no passive client exist
     * then don't send config event */
    ...
    bool is_passive_needed_a_g = ((!state->accel_info.desire_en && !state->gyro_info.desire_en) &&
                                  (state->passive_client_present & (ICM4X6XX_ACCEL | ICM4X6XX_GYRO)));
    bool is_passive_needed_t = (!state->sensor_temp_info.cur_sampling_rate_hz &&
                                (state->passive_client_present & ICM4X6XX_SENSOR_TEMP));

    ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "send cfg_evt passive need:%d/%d",
            is_passive_needed_a_g, is_passive_needed_t);
#else
    if(!state->accel_info.is_enabled && !state->gyro_info.is_enabled) {
        return;
    }
    ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "send cfg_evt");
#endif
    ...
    if ((state->accel_info.actual_publish && state->accel_info.desire_publish)) {
        ICM4X6XX_INST_PRINTF(LOW, instance, "a cfg_evt, new:%d, sr %d:%d, wmk %d:%d",
            state->a_new_stream,
        ...
        sns_time cfg_event_ts = sns_get_system_time();
        /* dae wmk will be the same(0) when DAE not avaiable */
        if((state->accel_info.last_sent_cfg.sample_rate == phy_sensor_config.sample_rate)
            && (state->accel_info.last_sent_cfg.dae_watermark == phy_sensor_config.DAE_watermark)
            && (state->accel_info.last_sent_cfg.range == phy_sensor_config.range[1])
            && (state->accel_info.last_sent_cfg.wmk == phy_sensor_config.water_mark)) {
        ...
        ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "a cfg_evt, ts %x%08x",
    ...
    /*
     * From QCOM: For the event-gated accel or passive sensor, the driver needs to send out a
     * physical_config_event with 0 sample rate in the payload and a sns_cal_event to let the
     * client know there is no data going to be sent.
     */
    if ((state->accel_info.gated_client_present && state->md_info.enable_md_int)) {
        phy_sensor_config.sample_rate = 0;
        ICM4X6XX_INST_PRINTF(LOW, instance, "gated cfg_evt, new:%d, sr %d:%d, wmk %d:%d",
        state->a_new_stream,
        (uint32_t)state->accel_info.last_sent_cfg.sample_rate, (uint32_t)phy_sensor_config.sample_rate,
        state->accel_info.last_sent_cfg.wmk, phy_sensor_config.water_mark);
        ...
        ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "gated cfg_evt ts %x%08x",
            (uint32_t)(cfg_event_ts >> 32), (uint32_t)cfg_event_ts);
            state->accel_info.last_sent_cfg.timestamp = cfg_event_ts;
            state->accel_info.last_sent_cfg.wmk = phy_sensor_config.water_mark;
            state->accel_info.last_sent_cfg.sample_rate = phy_sensor_config.sample_rate;
    ...
    if(state->gyro_info.actual_publish && state->gyro_info.desire_publish) {
        ...
        ICM4X6XX_INST_PRINTF(LOW, instance, "g cfg_evt, new:%d, range %d:%d, sr %d:%d, wmk %d:%d",
            state->g_new_stream,
            (uint32_t)state->gyro_info.last_sent_cfg.range, (uint32_t)phy_sensor_config.range[1],
        ...
        if((state->gyro_info.last_sent_cfg.sample_rate == phy_sensor_config.sample_rate)
            && (state->gyro_info.last_sent_cfg.dae_watermark == phy_sensor_config.DAE_watermark)
            && (state->gyro_info.last_sent_cfg.range == phy_sensor_config.range[1])
            && (state->gyro_info.last_sent_cfg.wmk == phy_sensor_config.water_mark)) {
            /* latch ts for old stream and config not changed */
        ...
        ICM4X6XX_INST_PRINTF(LOW, instance, "g cfg_evt, ts %x%08x",
            (uint32_t)(cfg_event_ts >> 32), (uint32_t)cfg_event_ts);
    ...
    if(state->sensor_temp_info.actual_publish) {
        ...
        ICM4X6XX_INST_PRINTF(LOW, instance, "t cfg_evt, sr %d:%d",
...
#endif
#ifdef ICM4X6XX_PASSIVE_SENSOR_SUPPORT
    if (!state->gyro_info.actual_publish &&
        !state->accel_info.actual_publish &&
        !state->sensor_temp_info.actual_publish &&
        (is_passive_needed_a_g | is_passive_needed_t)) {

        icm4x6xx_sensor_type passive_clients =
            state->passive_client_present & (ICM4X6XX_ACCEL | ICM4X6XX_GYRO | ICM4X6XX_SENSOR_TEMP);

        phy_sensor_config.active_current = 0;
        phy_sensor_config.sample_rate = 0;
        while (passive_clients != 0) {
            sns_sensor_uid *suid;
            if (passive_clients & ICM4X6XX_ACCEL) {
                sns_time cfg_event_ts = sns_get_system_time();
                suid = &state->accel_info.suid;
                passive_clients &= ~ICM4X6XX_ACCEL;
                if ((state->accel_info.last_sent_cfg_p.sample_rate == phy_sensor_config.sample_rate) &&
                    (state->accel_info.last_sent_cfg_p.dae_watermark == phy_sensor_config.DAE_watermark) &&
                    (0 != state->accel_info.last_sent_cfg_p.timestamp)) {
                    cfg_event_ts = state->accel_info.last_sent_cfg_p.timestamp;
                }
                pb_send_event(instance,
                              sns_std_sensor_physical_config_event_fields,
                              &phy_sensor_config,
                              ...
                              SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT,
                              suid);
                ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "psv(0x%x) a cfg_evt, sr:%d/%d, ts %x%08x",
                    passive_clients,
                    (uint32_t)state->accel_info.last_sent_cfg_p.sample_rate,
                    (uint32_t)phy_sensor_config.sample_rate,
                    (uint32_t)(cfg_event_ts >> 32), (uint32_t)cfg_event_ts);
                state->accel_info.last_sent_cfg_p.timestamp = cfg_event_ts;
                state->accel_info.last_sent_cfg_p.sample_rate = phy_sensor_config.sample_rate;
                state->accel_info.last_sent_cfg_p.dae_watermark = phy_sensor_config.DAE_watermark;
            } else if (passive_clients & ICM4X6XX_GYRO) {
                sns_time cfg_event_ts = sns_get_system_time();
                suid = &state->gyro_info.suid;
                passive_clients &= ~ICM4X6XX_GYRO;
                if ((state->gyro_info.last_sent_cfg_p.sample_rate == phy_sensor_config.sample_rate) &&
                    (state->gyro_info.last_sent_cfg_p.dae_watermark == phy_sensor_config.DAE_watermark) &&
                    (0 != state->gyro_info.last_sent_cfg_p.timestamp)) {
                    cfg_event_ts = state->gyro_info.last_sent_cfg_p.timestamp;
                }
                pb_send_event(instance,
                              sns_std_sensor_physical_config_event_fields,
                              &phy_sensor_config,
                              ...
                              SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT,
                              suid);
                ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "psv(0x%x) g cfg_evt, sr:%d/%d, ts %x%08x",
                    passive_clients,
                    (uint32_t)state->gyro_info.last_sent_cfg_p.sample_rate,
                    (uint32_t)phy_sensor_config.sample_rate,
                    (uint32_t)(cfg_event_ts >> 32), (uint32_t)cfg_event_ts);
                state->gyro_info.last_sent_cfg_p.timestamp = cfg_event_ts;
                state->gyro_info.last_sent_cfg_p.sample_rate = phy_sensor_config.sample_rate;
                state->gyro_info.last_sent_cfg_p.dae_watermark = phy_sensor_config.DAE_watermark;
            } else {
                sns_time cfg_event_ts = sns_get_system_time();
                suid = &state->sensor_temp_info.suid;
                passive_clients &= ~ICM4X6XX_SENSOR_TEMP;
                if ((state->sensor_temp_info.last_sent_cfg_p.sample_rate == phy_sensor_config.sample_rate) &&
                    (state->sensor_temp_info.last_sent_cfg_p.dae_watermark == phy_sensor_config.DAE_watermark) &&
                    (0 != state->sensor_temp_info.last_sent_cfg_p.timestamp)) {
                    cfg_event_ts = state->sensor_temp_info.last_sent_cfg_p.timestamp;
                }
                pb_send_event(instance,
                              sns_std_sensor_physical_config_event_fields,
                              &phy_sensor_config,
                              ...
                              SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT,
                              suid);
                ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "psv(0x%x) t cfg_evt, sr:%d/%d, ts %x%08x",
                    passive_clients,
                    (uint32_t)state->sensor_temp_info.last_sent_cfg_p.sample_rate,
                    (uint32_t)phy_sensor_config.sample_rate,
                    (uint32_t)(cfg_event_ts >> 32), (uint32_t)cfg_event_ts);
                state->sensor_temp_info.last_sent_cfg_p.timestamp = cfg_event_ts;
                state->sensor_temp_info.last_sent_cfg_p.sample_rate = phy_sensor_config.sample_rate;
                state->sensor_temp_info.last_sent_cfg_p.dae_watermark = phy_sensor_config.DAE_watermark;
            }
        }
    }
#endif /*ICM4X6XX_PASSIVE_SENSOR_SUPPORT*/
    //Save the state->md_info.op_mode_flag before decoding
    uint8_t prev_op_mode = state->md_info.op_mode;
    if (state->md_info.md_client_present) {
        if (state->gyro_info.is_enabled) {
            ...
            state->md_info.op_mode = 1;
        } else {
            op_mode_args.buf = &operating_mode_lpm[0];
            op_mode_args.buf_len = sizeof(operating_mode_lpm);
            state->md_info.op_mode = 0;
        }

        sns_time cfg_event_ts = sns_get_system_time();
        if(prev_op_mode == state->md_info.op_mode) {
            /* latch ts for old stream and config not changed */
            cfg_event_ts = state->md_info.last_sent_cfg.timestamp;
        }
        ...
        ICM4X6XX_INST_PRINTF(LOW, instance, "md cfg_evt");
        ...
        pb_send_event(instance,
                      ...
                      SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_PHYSICAL_CONFIG_EVENT,
                      &state->md_info.suid);
        state->md_info.last_sent_cfg.timestamp = cfg_event_ts;
    }
}
...
#if defined(ICM4X6XX_DISABLE_DAE) || defined(ICM4X6XX_DETECT_DAE_SENSOR)
...
sns_rc icm4x6xx_get_best_match_odr(
    ...
    uint8_t * restrict a_samples_to_discard,
    uint8_t * restrict g_samples_to_discard)
{
    uint8_t idx;
    ...
    for(idx = 0; idx < ARR_SIZE(inv_reg_map); idx++) {
        ...
        if(NULL != dae_if->ag.stream && dae_if->ag.stream_usable)
        {
...
#endif /* ICM4X6XX_DISABLE_DAE */
        {
#ifdef ICM4X6XX_ENABLE_PEDOMETER
            /* For HW pedometer, the minim ODR should be >= 50Hz */
            if (FLT_EQUAL(inv_reg_map[idx].odr, ICM4X6XX_ODR_25)) {
                continue;
            }
    ...
    *g_samples_to_discard = inv_reg_map[idx].g_discard_samples;
    ...
    if (FLT_EQUAL(desired_sample_rate, 1.0f)) {
        *a_samples_to_discard += 10;
    }

    /* Only for Yokohama testAccelerometer_25hz */
    if (((YOKOHAMA == state->product_info.series) || (YOKO_C1 == state->product_info.series))
        && FLT_EQUAL(desired_sample_rate, 25.0f)) {
        *a_samples_to_discard += 1;
    }

    /* Only for Yokohama testAccelerometer_15hz */
    if (((YOKOHAMA == state->product_info.series) || (YOKO_C1 == state->product_info.series))
        && FLT_EQUAL(desired_sample_rate, 15.0f)) {
        *a_samples_to_discard += 1;
    }

    /* Only for Yokohama testAccelerometer_10hz */
    if (((YOKOHAMA == state->product_info.series) || (YOKO_C1 == state->product_info.series))
        && FLT_EQUAL(desired_sample_rate, 10.0f)) {
        *a_samples_to_discard += 3;
    }

    /* Only for Yokohama testAccelerometer_5hz */
    if (((YOKOHAMA == state->product_info.series) || (YOKO_C1 == state->product_info.series))
        && FLT_EQUAL(desired_sample_rate, 5.0f)) {
...
void icm4x6xx_send_md_event(
     sns_sensor_instance *const instance,
     sns_motion_detect_event* event,
     sns_time event_timestamp)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)instance->state->state;
    ICM4X6XX_INST_PRINTF(LOW, instance, "md_event=%d",
     ...
     bool md_not_armed_event)
{
    ...
    if(enable || md_not_armed_event) {
        if (state->md_info.pre_md_state.motion_detect_event_type !=
            state->md_info.md_state.motion_detect_event_type) {
            icm4x6xx_send_md_event(instance, &state->md_info.md_state, sns_get_system_time());
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
void icm4x6xx_convert_and_send_step_counter_sample(
     sns_sensor_instance *const instance,
     sns_time timestamp,
     uint16_t sample_data)
{
    ...
    if (state->step_counter_info.desire_publish) {
        if(sample_data != 0)
        {
            float f_sample_data = (float)sample_data * 1.0f;

            ICM4X6XX_INST_PRINTF(MED, instance, "**/**/**/**/**/**/*step_counter cnt %d*/**/**/**/**/**/**", (uint32_t)f_sample_data);

            pb_send_sensor_stream_event(instance,
                                        &state->step_counter_info.suid,
                                        timestamp,
                                        SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT,
                                        status,
                                        &f_sample_data,
                                        1,
                                        state->encoded_sensor_pedometer_event_len);
            icm4x6xx_log_sensor_state_raw_one_axis(instance, true, true,
                &f_sample_data, timestamp, status);
...
bool icm4x6xx_need_flush_fifo(sns_sensor_instance *this)
{
    ...
    ICM4X6XX_INST_PRINTF(MED, this, "odr changed %d, a/g off_to_on %d %d, need_flush 0x%x",
...
void icm4x6xx_set_hw_wo_stop_fifo(sns_sensor_instance *this)
{
    ...
    if (state->g_off_to_on) {
        ...
        if(!state->gyro_info.postpone_enable) {
            icm4x6xx_en_gyro(this, state->gyro_info.desire_en);
        }
        #else
        icm4x6xx_en_gyro(this, state->gyro_info.desire_en);
        #endif /*ICM4X6XX_GYRO_RING_DOWN_WA*/
    }
}
...
void icm4x6xx_st_store_pre_state(sns_sensor_instance* this)
{
    ...
    #ifdef ICM4X6XX_ENABLE_PEDOMETER
    state->st_resume.en_pedometer = state->step_counter_info.is_enabled;
    state->st_resume.desire_en_pedometer = state->step_counter_info.desire_en;
    state->st_resume.desire_pub_pedometer =  state->step_counter_info.desire_publish;
    #endif
...
void icm4x6xx_create_heart_beat_timer(sns_sensor_instance *this, sns_time timeout_ticks)
{
    ...
    size_t req_len;
    /* QCOM: Should be bigger than sns_timer_sensor_config_size. Probably at least 64.*/
    uint8_t buffer[70];
...
#ifndef ICM4X6XX_DISABLE_DAE
    if (icm4x6xx_dae_if_available(this) && (state->fifo_info.max_requested_wmk == UINT32_MAX))
	{
        /* remove hb timer when flush only */
    ...
    if(req_len > 0) {
        ...
        if(state->heart_beat.timer_data_stream == NULL) {
            ...
            ICM4X6XX_INST_PRINTF(LOW, this, "hb timeout_period: %d ms",
                         tick2ms(req_payload.timeout_period));
#endif
        }
    } else {
        ICM4X6XX_INST_PRINTF(ERROR, this, "E");
...
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
void icm4x6xx_create_gyro_postpone_timer(sns_sensor_instance *this, sns_time timeout_ticks)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    sns_timer_sensor_config req_payload = sns_timer_sensor_config_init_default;
    size_t req_len;
    ...
    ICM4X6XX_INST_PRINTF(MED, this, "c g postpone timer");
    req_len = pb_encode_request(buffer, sizeof(buffer), &req_payload,
                    sns_timer_sensor_config_fields, NULL);
    if(req_len > 0) {
        if(state->gyro_info.timer_postpone_stream == NULL) {
            sns_service_manager *service_mgr = this->cb->get_service_manager(this);
            sns_stream_service *stream_mgr = (sns_stream_service*)
                    service_mgr->get_service(service_mgr, SNS_STREAM_SERVICE);
            stream_mgr->api->create_sensor_instance_stream(stream_mgr,
                  this,
                  state->timer_suid,
                  &state->gyro_info.timer_postpone_stream);
        }
        ...
        if(state->gyro_info.timer_postpone_stream == NULL) {
            ...
            state->gyro_info.timer_postpone_stream->api->send_request(state->gyro_info.timer_postpone_stream, &timer_req);
        }
    } else {
        ICM4X6XX_INST_PRINTF(ERROR, this, "E");
    }
...
#ifndef ICM4X6XX_DISABLE_DAE
    if (!icm4x6xx_dae_if_available(this) && (state->config_step == ICM4X6XX_CONFIG_FLUSHING_HW))
#else
...
#ifndef ICM406XX_ENABLE_A_LPM
    if (HAVANA == state->product_info.series || state->accel_info.en_fifo) {
        return ICM4X6XX_A_LNM;
    }
#else
    /* LPA keep the same way with Yoko */
    if (state->accel_info.en_fifo) {
        return ICM4X6XX_A_LNM;
    }
#endif
...
#endif /* ICM4X6XX_ENABLE_HIGHG */

#ifdef ICM4X6XX_ENABLE_PEDOMETER
    if (state->step_counter_info.desire_en) {
        return ICM4X6XX_A_LPM;
    }
#endif /* ICM4X6XX_ENABLE_PEDOMETER */
...
static void icm4x6xx_update_heart_beat(sns_sensor_instance *this, bool update)
{
    ...
    if(update) {
        ...
        #ifdef ICM4X6XX_GYRO_RING_DOWN_WA
        if(state->gyro_info.postpone_enable)
            hb_timeout += state->gyro_info.postpone_tick;
        #endif
    }
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
void icm4x6xx_reconfig_pedometer(sns_sensor_instance *this)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;

    if (state->step_counter_info.desire_en) {
        if(!state->step_counter_info.is_enabled) {
            icm4x6xx_enable_hw_pedometer(this, true);
            icm4x6xx_start_step_counter_polling_timer(this);
        }
    } else {
        if(state->step_counter_info.is_enabled) {
            icm4x6xx_enable_hw_pedometer(this, false);
            state->step_counter_info.is_enabled = false;
        }
        if(state->step_counter_info.is_timer_active) {
            sns_sensor_util_remove_sensor_instance_stream(this, &state->step_counter_info.timer_data_stream);
            state->step_counter_info.is_timer_active = false;
        }
    }
}
#endif
...
void icm4x6xx_reconfig_hw(sns_sensor_instance *this)
{
...
#ifndef ICM4X6XX_YOKO_C1_DMP_MIN_500HZ
    float req_acc_odr = FLT_EQUAL(0, state->accel_info.req_match_odr) ? ICM4X6XX_ODR_50 : state->accel_info.req_match_odr;
#else   //TODO 500Hz too sensitive for MD?
    float req_acc_odr = FLT_EQUAL(0, state->accel_info.req_match_odr) ? ICM4X6XX_ODR_500 : state->accel_info.req_match_odr;
#endif
    ...
    if (state->md_info.md_client_present &&
        ...
        icm4x6xx_send_md_event(this, &state->md_info.md_state, sns_get_system_time());
    ...
    if (rc == SNS_RC_SUCCESS) {
        ...
        #ifdef ICM4X6XX_GYRO_RING_DOWN_WA
        /* do not send config event when need to postpone enable gyro */
        state->gyro_info.actual_publish = state->gyro_info.desire_publish & !state->gyro_info.postpone_enable;
        #else
    ...
    ICM4X6XX_INST_PRINTF(HIGH, this, "a/g/t desire/actual pub %d %d %d/%d %d %d, irq rdy %d, 1st_sample %d, rc %d",
                         state->accel_info.desire_publish,
                         state->gyro_info.desire_publish,
                         state->sensor_temp_info.desire_publish,
                         state->accel_info.actual_publish,
...
#endif
        state->fac_test_in_progress) {
            icm4x6xx_dae_if_start_ag_streaming(this);
            icm4x6xx_dae_if_start_temp_streaming(this);
        } else {
            icm4x6xx_dae_if_stop_ag_streaming(this);
            icm4x6xx_dae_if_stop_temp_streaming(this);
        }
    }
#endif
    ...
    icm4x6xx_update_heart_beat(this, true);
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    icm4x6xx_reconfig_pedometer(this);
#endif
            ...
            bool is_unreliable)
{
    ...
    ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "g filter delay: %d",
    ...
    if (is_hi_res != state->product_info.use_hi_res) {
        ICM4X6XX_INST_PRINTF(HIGH, instance, "hi_res bit mismatch %d:%d",
    ...
    if(state->fac_test_in_progress && state->fac_test_sensor == ICM4X6XX_GYRO) {
        ...
        for(i = 0; state->fac_test_info.num_samples > FACTORY_CAL_DISCARD_G && i < TRIAXIS_NUM; i++) {
            ...
            ICM4X6XX_INST_PRINTF(HIGH, instance, "[INVN]Sample opdata_raw:%d", (int32_t)(opdata_raw[i]*1000000));
            ICM4X6XX_INST_PRINTF(HIGH, instance, "[INVN]Sample sample_sum:%d,    sample_square_sum:%d",
                        (int32_t)(state->fac_test_info.sample_sum[i]*1000000),
                        (int32_t)(state->fac_test_info.sample_square_sum[i]*1000000));
        }
    }
    ...
    if (!state->fac_test_in_progress &&
        ...
        ICM4X6XX_INST_PRINTF(HIGH, instance, "g_sample %d, TS: %x%08x, LSB: %d %d %d, %d",
                             state->g_reported_sample,
            ...
            bool is_unreliable)
{
    ...
    ICM4X6XX_VERB_INST_PRINTF(LOW, instance, "a filter delay: %d",
    ...
    if (!state->fac_test_in_progress &&
        ...
        ICM4X6XX_INST_PRINTF(HIGH, instance, "a_sample %d, TS: %x%08x, LSB: %d %d %d, %d",
                             state->a_reported_sample,
                             (uint32_t)(timestamp >> 32), (uint32_t)timestamp,
                             raw_lsb_32b[TRIAXIS_X], raw_lsb_32b[TRIAXIS_Y], raw_lsb_32b[TRIAXIS_Z],
                             status);
...
#ifndef ICM4X6XX_DISABLE_DAE
    if (state->is_flush || (icm4x6xx_dae_if_available(instance) && state->dae_ts_type == sns_dae_timestamp_type_SNS_DAE_TIMESTAMP_TYPE_SYSTEM_TIME)) {
#else
    if (state->is_flush) {
#endif
        *matched_cnt = packet_cnt;
            ...
            sns_time *meas_intvl)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state *)instance->state->state;
    sns_time cal_end = 0;
    double estimated_intvl = 0.0f;
    double measured_interval = 0.0f;

#ifndef ICM4X6XX_DISABLE_DAE
    /* data_age_limit_ticks = state->fifo_info.cur_max_flush_ticks * 1.1 */
    if (icm4x6xx_dae_if_available(instance) && state->fifo_info.flush_only &&
...
#ifdef ICM4X6XX_DEBUG_VERBOSE
    ICM4X6XX_INST_PRINTF(LOW, instance, "intl_cnt_20b, last_rec_cnt_16b: %#x %#x, last_rec_cnt: %#x %#x %#x ",
                         intl_cnt_20b, last_rec_cnt_16b,
                         last_rec_cnt_20b, last_rec_cnt_b16_b19, last_rec_cnt_16b);
    ICM4X6XX_INST_PRINTF(MED, instance, "delta_cnt: %d, intl_cnt_strobe_ts: %x%08x, last_rec_ts: %x%08x, intnl cnt: %d",
                         delta_cnt,
                         (uint32_t)(intl_cnt_strobe_ts >> 32), (uint32_t)intl_cnt_strobe_ts,
                         (uint32_t)(*last_rec_ts >> 32), (uint32_t)*last_rec_ts, intl_cnt_20b);
#endif
}
     ...
     sns_time            timestamp)
{
    ...
    icm4x6xx_dump_fifo(instance, buf, buf_len);
    #endif
    ...
    if (packet_cnt != valid_buf_len / packet_size) {
        header.head_byte = buf[0];
        ICM4X6XX_INST_PRINTF(HIGH, instance, "mixed_packet_err, fifo_head=0x%x, fifo_format=%d/%d",
            header.head_byte, state->fifo_info.curr_format, state->fifo_info.desire_format);

        /* FIFO format mismatch, correct curr_format to real fifo format
         * TODO: 1) find the root cause of the mismatch, or
         *       2) call icm4x6xx_ff_format_check in icm4x6xx_send_ag_config
         *       run GYR-234 to reproduce.
         */
        if ((false == header.bits.gyro_bit) &&
            (ICM4X6XX_FORMAT_16_BYTES == state->fifo_info.curr_format) &&
            (ICM4X6XX_FORMAT_ACCEL_8_BYTES == state->fifo_info.desire_format)) {
            state->fifo_info.curr_format = state->fifo_info.desire_format;
        }
        if ((false == header.bits.accel_bit) &&
            (ICM4X6XX_FORMAT_16_BYTES == state->fifo_info.curr_format) &&
            (ICM4X6XX_FORMAT_GYRO_8_BYTES == state->fifo_info.desire_format)) {
            state->fifo_info.curr_format = state->fifo_info.desire_format;
        }
    }
    ...
    icm4x6xx_update_heart_beat(instance, true);
    ...
    while (idx < valid_buf_len) {
        ...
        ICM4X6XX_VERB_INST_PRINTF(HIGH, instance, "header: %#x, a/g res:%d/%d", header.head_byte,
            (uint32_t)(ACC_RES_M2_PER_S(state->accel_info.fsr, header.bits.twentybits_bit,
                                         state->product_info.hi_range) * 1000 * 1000),
            (uint32_t)(GYRO_RES_RPS(state->gyro_info.fsr, header.bits.twentybits_bit,
                                         state->product_info.hi_range) * 1000 * 1000));
        if (header.head_byte == 0xff ||
            header.head_byte == 0x80 ||
            header.head_byte == 0x00) {
        ...
        if (report_accel) {
#ifdef ICM4X6XX_ODR_CHANGING_SAMPLE_MISSING_SAMPLE_GAP
            /* only add this wa when dae_flush_period/data_age_limit_ticks not 0, TODO: DAE only? */
            ICM4X6XX_INST_PRINTF(MED, instance, "prev/current flush ticks %u %u",
                state->fifo_info.prev_max_requested_flush_ticks, state->fifo_info.max_requested_flush_ticks);

            if (!added_one_more_sample &&
                state->fifo_info.prev_max_requested_flush_ticks &&
                ...
                while (add_n_samples &&  max_samples < 5) {
                    icm4x6xx_handle_accel_sample(instance, accel_raw, header.bits.twentybits_bit,
                        accel_high_res, sample_ts - meas_intvl * add_n_samples, true, true, true);
                    add_n_samples--;
                    max_samples++;
                }
            }
...
#endif
            icm4x6xx_handle_accel_sample(instance, accel_raw, header.bits.twentybits_bit,
                accel_high_res, sample_ts, is_first_one, is_last_one, false);
        }
...
#endif
            icm4x6xx_handle_gyro_sample(instance, gyro_raw, header.bits.twentybits_bit,
                gyro_high_res, sample_ts, is_first_one, is_last_one, false);
        }
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
void icm4x6xx_start_step_counter_polling_timer(sns_sensor_instance *this)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    sns_timer_sensor_config req_payload = sns_timer_sensor_config_init_default;
    /* QCOM: Should be bigger than sns_timer_sensor_config_size. Probably at least 64.*/
    ...
    if(NULL == state->step_counter_info.timer_data_stream) {
        sns_service_manager *smgr = this->cb->get_service_manager(this);
        sns_stream_service *strm_svc =
            (sns_stream_service*)smgr->get_service(smgr, SNS_STREAM_SERVICE);
        rc = strm_svc->api->create_sensor_instance_stream(strm_svc, this,
            state->timer_suid, &state->step_counter_info.timer_data_stream);
    }

    if (rc != SNS_RC_SUCCESS ||
        NULL == state->step_counter_info.timer_data_stream) {
        ICM4X6XX_INST_PRINTF(ERROR, this, "start_step_counter_polling_timer error %d", rc);
        return;
    }

    req_payload.is_periodic = true;
    req_payload.start_time = sns_get_system_time();
    /* force 5Hz, TODO: dynamic change(from request and save to registry) like sensor_temp */
    //req_payload.timeout_period = state->step_counter_info.sampling_intvl;
    req_payload.timeout_period = sns_convert_ns_to_ticks(200 * 1000 * 1000);

    ICM4X6XX_INST_PRINTF(HIGH, this, "step_counter timeout period: %x%08x",
                     (uint32_t)(req_payload.timeout_period >> 32),
                        (uint32_t)req_payload.timeout_period);

    ICM4X6XX_INST_PRINTF(HIGH, this, "start timer for step_counter");
    timer_req.request_len = pb_encode_request(buffer,
                                              sizeof(buffer),
                                              &req_payload,
                                              sns_timer_sensor_config_fields,
                                              NULL);
    if(timer_req.request_len > 0) {
        state->step_counter_info.timer_data_stream->api->send_request(state->step_counter_info.timer_data_stream, &timer_req);
        state->step_counter_info.cur_sampling_rate_hz = state->step_counter_info.sampling_rate_hz;
        state->step_counter_info.is_timer_active = true;
    }
}

/* sample_raw keep unchange and accel samples keep change */
bool icm4x6xx_pedometer_suspend_detaction(sns_sensor_instance *const this, uint16_t step_raw)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    ...
    icm4x6xx_read_accel_data(this, data_temp);
    state->step_counter_info.a_sample_value[state->step_counter_info.cnt] = (data_temp[0]>0) ? data_temp[0] : -data_temp[0] ;

    ICM4X6XX_INST_PRINTF(LOW, this, "icm4x6xx_pedometer_suspend_detaction step=%d, cnt=%d, ax=%d, sum=%d",
                            step_raw, state->step_counter_info.cnt, data_temp[0], sum);


    if(24 == state->step_counter_info.cnt) {
        state->step_counter_info.cnt = 0;
        if ((sum >= 10000) && (step_raw == 0)) {
            sum = 0;
            return true;
        } else {
            ICM4X6XX_INST_PRINTF(LOW, this, "icm4x6xx_pedometer_suspend_detaction-- %d", sum);
            sum = 0;
            return false;
        }
     } else {
        sum += state->step_counter_info.a_sample_value[state->step_counter_info.cnt];
        if (step_raw != 0) {
            state->step_counter_info.cnt = 0;
            return false;
        }
    ...
    state->step_counter_info.cnt = (state->step_counter_info.cnt+1)%25;

    return false;

}

void icm4x6xx_handle_step_counter_sample(
    sns_sensor_instance *const instance,
    uint64_t timeout_ticks)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)instance->state->state;
    ...
    icm4x6xx_read_step_cnt(instance, &sample_raw);

    #ifdef ICM4X6XX_ENABLE_PEDOMETER_DEBUG_LOG
    ICM4X6XX_INST_PRINTF(HIGH, instance, "icm4x6xx_handle_step_counter_sample: sample_raw=%d step_counter_cnt=%d",
                                sample_raw, state->step_counter_info.step_counter_cnt);

    ICM4X6XX_INST_PRINTF(MED, instance, "step_counter_info desire_en=%d, en=%d, timer=%d, clients=%d, init_status=%d",
                                state->step_counter_info.desire_en,
                                state->step_counter_info.is_enabled,
                                state->step_counter_info.is_timer_active,
                                state->step_counter_info.client_cnt,
                                state->step_counter_info.dmp_init_status);

#ifdef ICM4X6XX_DEBUG_VERBOSE
    icm4x6xx_dump_reg(instance);
    icm4x6xx_dump_pedometer_reg(instance);
#endif
    uint8_t int_status, int_status2, int_status3;
    icm4x6xx_read_int_status3(instance, &int_status3);
    icm4x6xx_read_int_status2(instance, &int_status2);
    icm4x6xx_read_int_status(instance, &int_status);
    ICM4X6XX_INST_PRINTF(LOW, instance, "int regs status 0x%x 0x%x 0x%x", int_status, int_status2, int_status3);

    /* step counter keep 0 and accel keeps change */
    if(icm4x6xx_pedometer_suspend_detaction(instance, sample_raw))
    //if(state->step_counter_info.is_enabled)
    {
        ICM4X6XX_INST_PRINTF(LOW, instance, "icm4x6xx_pedometer_suspend_detaction++ done");
        /*dump dmp status */
        uint8_t dmp_status = 0;
        int rc = icm4x6xx_dump_check_sram(instance, true, &dmp_status);
        if(SNS_RC_SUCCESS != rc)
            ICM4X6XX_INST_PRINTF(ERROR, instance, "icm4x6xx_sram_dump_show failure");

        /* reset dmp, works but is workaround */
        //icm4x6xx_en_apex_hw(instance);
        //icm4x6xx_create_en_dmp_timer(instance, sns_convert_ns_to_ticks(DMP_START_UP_TIME));
    }
    #endif /* ICM4X6XX_ENABLE_PEDOMETER_DEBUG_LOG */

    if(sample_raw != state->step_counter_info.step_counter_cnt)
    {
        icm4x6xx_convert_and_send_step_counter_sample(instance, timeout_ticks, sample_raw);
        state->step_counter_info.step_counter_cnt = sample_raw;
    }
}
#endif  /* ICM4X6XX_ENABLE_PEDOMETER */
...
#ifndef ICM4X6XX_DISABLE_DAE
    if (!icm4x6xx_dae_if_available(instance) && state->product_info.series == HAVANA)
#else
    if (state->product_info.series == HAVANA)
#endif
        state->gyro_info.en_fifo = true;
            ...
            if (state->fifo_info.curr_format == state->fifo_info.desire_format) {
				...
				if(icm4x6xx_dae_if_available(instance))
				{
			        icm4x6xx_dae_if_start_ag_streaming(instance);
			        icm4x6xx_dae_if_start_temp_streaming(instance);
				}
				else
#endif
				{
...
static sns_rc icm4x6xx_validate_sensor_temp_odr(icm4x6xx_instance_state *state)
{
    ...
    if (rc == SNS_RC_SUCCESS) {
        state->sensor_temp_info.sampling_intvl =
          sns_convert_ns_to_ticks(1000000000.0f / state->sensor_temp_info.sampling_rate_hz);
    }
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
static sns_rc icm4x6xx_validate_step_counter_odr(icm4x6xx_instance_state *state)
{
    sns_rc rc = SNS_RC_SUCCESS;

    if(state->step_counter_info.sampling_rate_hz <= ICM4X6XX_STEP_COUNTER_ODR_1) {
        state->step_counter_info.sampling_rate_hz = ICM4X6XX_STEP_COUNTER_ODR_1;
    } else if (state->step_counter_info.sampling_rate_hz > ICM4X6XX_STEP_COUNTER_ODR_1 &&
               state->step_counter_info.sampling_rate_hz <= ICM4X6XX_STEP_COUNTER_ODR_5) {
        state->step_counter_info.sampling_rate_hz = ICM4X6XX_STEP_COUNTER_ODR_5;
    } else {
        state->step_counter_info.sampling_intvl = 0;
        rc = SNS_RC_NOT_SUPPORTED;
    }

    if (rc == SNS_RC_SUCCESS) {
        state->step_counter_info.sampling_intvl =
          sns_convert_ns_to_ticks(1000000000.0f / state->step_counter_info.sampling_rate_hz);
    }
...
static void icm4x6xx_process_interrupt_data_stream(sns_sensor_instance *const this)
{
    ...
    while(NULL != event) {//level trigger mode, avoid infinite loop
        if (event->message_id == SNS_INTERRUPT_MSGID_SNS_INTERRUPT_REG_EVENT) {
            ...
            /* read out all interrupt events for removing duplicated message, TODO: wait for reproduce */
            pb_istream_t stream;
            while(event) {
            ...
            if (pb_decode(&stream, sns_interrupt_event_fields, &irq_event)) {
                ...
                ICM4X6XX_INST_PRINTF(ERROR, this, "event decode fail or bad event ts");
#ifdef ICM4X6XX_DEBUG_VERBOSE
...
static void icm4x6xx_process_sensor_temperature_data_stream(sns_sensor_instance *const this)
{
    ...
    if(NULL == state->timer_data_stream) {
        ICM4X6XX_VERB_INST_PRINTF(MED, this, "t ds not created!");
        return;
    }
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
static void icm4x6xx_process_step_counter_data_stream(sns_sensor_instance *const this)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    sns_sensor_event *event;

    if(NULL == state->step_counter_info.timer_data_stream) {
        ICM4X6XX_VERB_INST_PRINTF(MED, this, "step_counter ds not created!");
        return;
    }

    event = state->step_counter_info.timer_data_stream->api->peek_input(state->step_counter_info.timer_data_stream);
    while(NULL != event) {
        pb_istream_t stream = pb_istream_from_buffer((pb_byte_t*)event->event,
                                                 event->event_len);
        if(event->message_id == SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT) {
            sns_timer_sensor_event timer_event;
            if(pb_decode(&stream, sns_timer_sensor_event_fields, &timer_event)) {
                //received step_counter timer event
                ICM4X6XX_VERB_INST_PRINTF(LOW, this, "received step_counter timer event");
                if (state->step_counter_info.desire_en &&
                    state->step_counter_info.is_timer_active &&
                    state->step_counter_info.sampling_intvl > 0) {
                    icm4x6xx_handle_step_counter_sample(this, timer_event.timeout_time);
                } else {
                    ICM4X6XX_VERB_INST_PRINTF(LOW, this, "remove timer stream");
                    sns_sensor_util_remove_sensor_instance_stream(this, &state->step_counter_info.timer_data_stream);
                    state->step_counter_info.is_timer_active = false;
                    break;
                }
        ...
        event = state->step_counter_info.timer_data_stream->api->get_next_input(state->step_counter_info.timer_data_stream);
    }
}
#endif /* ICM4X6XX_ENABLE_PEDOMETER */
...
static void icm4x6xx_process_heart_beat_data_stream(sns_sensor_instance *const this)
{
    ...
    if(NULL == state->heart_beat.timer_data_stream) {
        ICM4X6XX_VERB_INST_PRINTF(MED, this, "hb ds not created!");
        return;
    }
    ...
    while(NULL != event) {
        ...
        if(event->message_id == SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT) {
            ...
            if(pb_decode(&stream, sns_timer_sensor_event_fields, &timer_event)) {
                if ((state->accel_info.actual_publish || state->gyro_info.actual_publish) ||
                    (state->accel_info.gated_client_present && !state->md_info.enable_md_int)) {
                    ICM4X6XX_INST_PRINTF(LOW, this, "receive hb timer event, %d", state->heart_beat.heart_attack_cnt);
                    if (SNS_RC_SUCCESS != icm4x6xx_dump_reg(this)) {
                        /* force soft reset when bus communicate fail */
                        state->heart_beat.heart_attack_cnt = 2;
                    }
                    if(state->heart_beat.heart_attack_cnt >= MAX_HEART_ATTACKS) {
                        ICM4X6XX_INST_PRINTF(ERROR, this, "heart_attack: Max reset tried.");
                        sns_sensor_util_remove_sensor_instance_stream(this, &state->heart_beat.timer_data_stream);
                        state->heart_beat.heart_attack_cnt = 0;
                    } else if(state->heart_beat.heart_attack_cnt >= 2) {
        ...
        if(NULL == state->heart_beat.timer_data_stream)
        {
            ICM4X6XX_INST_PRINTF(MED, this, "hb ds has been removed!");
            return;
        }
...
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
static void icm4x6xx_process_gyro_postpone_stream(sns_sensor_instance *const this)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    sns_sensor_event *event;

    if (NULL == state->gyro_info.timer_postpone_stream) {
        ICM4X6XX_VERB_INST_PRINTF(MED, this, "gyro postpone null");
        return;
    }

    event = state->gyro_info.timer_postpone_stream->api->peek_input(state->gyro_info.timer_postpone_stream);
    while(NULL != event) {
        pb_istream_t stream = pb_istream_from_buffer((pb_byte_t*)event->event,
                                                 event->event_len);
        if(event->message_id == SNS_TIMER_MSGID_SNS_TIMER_SENSOR_EVENT) {
            sns_timer_sensor_event timer_event;
            if(pb_decode(&stream, sns_timer_sensor_event_fields, &timer_event)) {
                ICM4X6XX_VERB_INST_PRINTF(LOW, this, "receive postpone g timer event %d",
                    state->gyro_info.postpone_enable);
                if (state->gyro_info.postpone_enable) {
                    if(SNS_RC_SUCCESS == icm4x6xx_en_gyro(this, true)) {
                        state->gyro_info.postpone_enable = false;
                        /* only send config event after enable gyro */
                        state->gyro_info.actual_publish =
                            state->gyro_info.desire_publish & !state->gyro_info.postpone_enable;
                        icm4x6xx_send_config_event(this);
                    }
                } else {
                    ICM4X6XX_VERB_INST_PRINTF(LOW, this, "remove postpone g stream");
                    sns_sensor_util_remove_sensor_instance_stream(this, &state->gyro_info.timer_postpone_stream);
                    break;
                }
            }
        } else if(event->message_id == SNS_TIMER_MSGID_SNS_TIMER_SENSOR_REG_EVENT) {
            ICM4X6XX_VERB_INST_PRINTF(LOW, this, "postpone g timer reg");
        } else {
            ICM4X6XX_VERB_INST_PRINTF(LOW, this, "postpone g unkown msg id %d", event->message_id);
        }
        event = state->gyro_info.timer_postpone_stream->api->get_next_input(state->gyro_info.timer_postpone_stream);
    }
}

static void icm4x6xx_gyro_postpone_config(sns_sensor_instance *const this)
{
    icm4x6xx_instance_state *state = (icm4x6xx_instance_state*)this->state->state;
    sns_time delta = sns_get_system_time() - state->gyro_info.last_off_tick;
    ICM4X6XX_VERB_INST_PRINTF(HIGH, this, "postpone cfg %d, %x%08x, %d %d",
            state->gyro_info.postpone_enable,
            (uint32_t)(delta >> 32), (uint32_t)delta,
            state->gyro_info.desire_en, !state->gyro_info.is_enabled);
    if((true == state->gyro_info.desire_en) &&
       (false == state->gyro_info.postpone_enable) &&
       (sns_convert_ns_to_ticks(GYRO_RING_DOWN_TIME) > delta) &&
       (state->gyro_info.desire_en && !state->gyro_info.is_enabled)) {
        state->gyro_info.postpone_enable = true;
        state->gyro_info.postpone_tick = sns_convert_ns_to_ticks(GYRO_RING_DOWN_TIME) - delta;
        ICM4X6XX_INST_PRINTF(HIGH, this, "postpone en gyro %d, %x%08x",
            state->gyro_info.postpone_enable,
            (uint32_t)(state->gyro_info.postpone_tick >> 32), (uint32_t)(state->gyro_info.postpone_tick));
        icm4x6xx_create_gyro_postpone_timer(this, state->gyro_info.postpone_tick);
    }
}
#endif /*ICM4X6XX_GYRO_RING_DOWN_WA*/

void icm4x6xx_publish_error(
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    } else if (ICM4X6XX_STEP_COUNTER == sensor_type) {
        suid = &state->step_counter_info.suid;
#endif
    } else {
        return;
    }
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
        icm4x6xx_process_step_counter_data_stream(this);
#endif
#endif
    }
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
    icm4x6xx_process_gyro_postpone_stream(this);
#endif
              ...
              sns_request const *client_request)
{
    ...
    float desired_wakeup_sdc_rate = 0.0f;
    float req_match_odr = 0.0f;
    ...
    ICM4X6XX_INST_PRINTF(MED, this, "drv version 0x%06x",
    ...
    if(client_request->message_id == SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_CONFIG) {
        ...
        ICM4X6XX_INST_PRINTF(MED, this, "desire sample/report/wakeup_sdc rate %d/100 %d/100 %d/100",
                             (uint32_t)(desired_sample_rate * 100), (uint32_t)(desired_report_rate * 100),
                             (uint32_t)(desired_wakeup_sdc_rate * 100));
         ...
         *   ERROR log level   lead to acceptance test FAIL:"  M-EF   error/fatal messages"
         */
        if((rv != SNS_RC_SUCCESS ||
           (FLT_EQUAL(0, desired_sample_rate) && !state->md_info.md_client_present))) {
            /* sensor_temp and step counter depends on accel too */
            if (state->accel_info.desire_en
                #ifdef ICM4X6XX_ENABLE_PEDOMETER
                && !state->step_counter_info.desire_publish
                #endif
                && !state->sensor_temp_info.desire_publish) {
                state->accel_info.desire_en = false;
                state->accel_info.desire_publish = false;
                icm4x6xx_publish_error(this, ICM4X6XX_ACCEL, SNS_STD_ERROR_INVALID_VALUE);
                ICM4X6XX_INST_PRINTF(HIGH, this, "Unsupported A ODR rv=%d", rv);
            }
            if (state->gyro_info.desire_en) {
                state->gyro_info.desire_en = false;
                state->gyro_info.desire_publish = false;
                icm4x6xx_publish_error(this, ICM4X6XX_GYRO, SNS_STD_ERROR_INVALID_VALUE);
                ICM4X6XX_INST_PRINTF(HIGH, this, "Unsupported G ODR rv=%d", rv);
            }
        }
        ...
        if(state->sensor_temp_info.desire_publish) {
            ...
            if(rv != SNS_RC_SUCCESS) {
                ...
                ICM4X6XX_INST_PRINTF(HIGH, this, "Unsupported T ODR rv=%d", rv);
                state->sensor_temp_info.desire_publish = false;
                icm4x6xx_publish_error(this, ICM4X6XX_SENSOR_TEMP, SNS_STD_ERROR_INVALID_VALUE);
            }
        } else {
            state->sensor_temp_info.sampling_intvl = 0;
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
        if(state->step_counter_info.desire_publish) {
            rv = icm4x6xx_validate_step_counter_odr(state);
            if(rv != SNS_RC_SUCCESS) {
                //step_counter ODR match error
                ICM4X6XX_INST_PRINTF(HIGH, this, "Unsupported pedometer ODR rv=%d", rv);
                icm4x6xx_publish_error(this, ICM4X6XX_STEP_COUNTER, SNS_STD_ERROR_INVALID_VALUE);
            }
        } else {
            state->step_counter_info.sampling_intvl = 0;
            state->step_counter_info.sampling_rate_hz = 0.0f;
        }
#endif
...
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
        icm4x6xx_gyro_postpone_config(this);
#endif
