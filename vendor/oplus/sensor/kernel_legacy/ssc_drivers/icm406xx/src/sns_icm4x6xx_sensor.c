...
#ifdef ICM4X6XX_GET_PARAMETER_FROM_SMEM
...
#define CONFIG_STEP_COUNTER     ".step_counter.config"
#endif
...
#define ICM4X6XX_FAC_CAL_BIAS    ".bias"
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    ICM4X6XX_GEN_GROUP(0, CONFIG_STEP_COUNTER),
#endif
    ...
    else
    {
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
                      group_name.buf_len) ||
         0 == strncmp((char*)group_name.buf, icm4x6xx_reg_config[reg_idx][REG_CONFIG_STEP_COUNTER],
#endif
                      group_name.buf_len))
      {
        {
        ...
        if(rv)
        {
          ...
          if (state->sensor == ICM4X6XX_ACCEL)
          {
              ...
              sensor = this->cb->get_library_sensor(this, false))
            {
              ...
              ICM4X6XX_PRINTF(HIGH, this, "(0x%x)Registry acc_cfg res_idx %d",
          ...
          else if (state->sensor == ICM4X6XX_GYRO)
          {
              ...
              sensor = this->cb->get_library_sensor(this, false))
            {
              ...
              ICM4X6XX_PRINTF(HIGH, this, "(0x%x)Registry gyro_cfg res_idx %d",
                sensor_state->sensor, sensor_state->common.g_res_idx);
            }
          }

          ICM4X6XX_PRINTF(LOW, this, "(0x%x)Registry "
                     "is_dri:%d, hw_id:%d resolution_idx:%d, supports_sync_stream:%d",
                     state->sensor,
                           ...
                           group_name.buf_len))
      {
        ...
        if(rv)
        {
          ...
          ICM4X6XX_PRINTF(LOW, this, "(0x%x)Registry com_cfg "
                     "bus_type:%d bus_inst:%d slave_control:%d "
                     "speed_KHz min:%d max:%d, reg_addr_type:%d "
                     "addr: i2c:0x%x i3c:0x%x",
                     state->sensor,
          ...
          ICM4X6XX_PRINTF(LOW, this, "(0x%x)Registry irq_cfg "
                     "num:%d pull_type:%d is_chip_pin:%d "
                     "drive_strength:%d trigger_type:%d rigid:%d",
                     state->sensor,
                           ...
                           group_name.buf_len))
      {
        ...
        if(rv)
        {
          ...
          ICM4X6XX_PRINTF(LOW, this, "(0x%x)Registry comm_placement p[0]:%u p[6]:%u p[11]:%u",
                     state->sensor,
                     (uint32_t)state->common.placement[0],
                     (uint32_t)state->common.placement[6],
                     (uint32_t)state->common.placement[11]);
        }
      }
                           ...
                           group_name.buf_len))
      {
        ...
        {
#ifdef ICM4X6XX_GET_PARAMETER_FROM_SMEM
          uint8_t tdk_gsensor_dir;
          struct sensor_direction gsensor_dir;
          struct sensor_hw *acc_hw;

          oppo_get_sensor_hw(OPPO_ACCEL, ICM4X6XX, &acc_hw);
          if (acc_hw != NULL && acc_hw->direction != DEFAULT_CONFIG)
          {
              tdk_gsensor_dir = acc_hw->direction;
              get_direction(tdk_gsensor_dir, &gsensor_dir);
              state->common.registry_orient_received = true;
              state->common.axis_map[0].opaxis = gsensor_dir.map[0];
              state->common.axis_map[0].invert = gsensor_dir.sign[0];
              state->common.axis_map[1].opaxis = gsensor_dir.map[1];
              state->common.axis_map[1].invert = gsensor_dir.sign[1];
              state->common.axis_map[2].opaxis = gsensor_dir.map[2];
              state->common.axis_map[2].invert = gsensor_dir.sign[2];
              SNS_PRINTF(HIGH, this, "tdk_gsensor_dir: %d", tdk_gsensor_dir);
          }
#endif
          ...
          ICM4X6XX_PRINTF(LOW, this, "Input Axis:%d maps to Output Axis:%d with inversion %d",
              ...
              0 == strncmp((char*)group_name.buf,
                         ...
                         group_name.buf_len))
      {
        ICM4X6XX_VERB_PRINTF(MED, this, "Read group_name from registry: %c%c%c%c%c",
            ((char *)(group_name.buf))[20],
            ((char *)(group_name.buf))[21],
            ((char *)(group_name.buf))[22],
            ((char *)(group_name.buf))[23],
            ((char *)(group_name.buf))[24]
            );
        ...
        if(rv)
        {
          ...
          ICM4X6XX_PRINTF(MED, this, "(%#X)Read bias from registry: %d/1M %d/1M %d/M, ver:%d",
                    state->sensor,
                    (int32_t)(state->fac_cal_bias[TRIAXIS_X]*1000000),
                    (int32_t)(state->fac_cal_bias[TRIAXIS_Y]*1000000),
                    (int32_t)(state->fac_cal_bias[TRIAXIS_Z]*1000000),
                           ...
                           group_name.buf_len))
      {
        ...
        if(rv)
        {
          ...
          ICM4X6XX_PRINTF(HIGH, this, "(%#X)MD config(disable:%d, threshold:%d/1k m/s2)",
              state->sensor,
              state->md_config.disable, (int32_t)(state->md_config.thresh*1000));
        }
      }
      else
      {
...
#endif
#ifdef ICM4X6XX_ENABLE_PEDOMETER
    else if(ICM4X6XX_STEP_COUNTER == state->sensor)
    {
      icm4x6xx_sensor_send_registry_request(this, icm4x6xx_reg_config[reg_idx][REG_CONFIG_STEP_COUNTER]);
    }
#endif
  }
}
        ...
        sns_sensor_instance *const instance, icm4x6xx_sensor_type sensor)
{
  ...
  ICM4X6XX_PRINTF(LOW, this, "(%#X)update_registry reg_idx %d", state->sensor, reg_idx);
  ...
  ICM4X6XX_VERB_PRINTF(MED, this, "registry name_data:%c%c%c%c%c",
    ((char *)(name_data.buf))[20],
    ((char *)(name_data.buf))[21],
    ((char *)(name_data.buf))[22],
    ((char *)(name_data.buf))[23],
    ((char *)(name_data.buf))[24]
    );
  ICM4X6XX_VERB_PRINTF(LOW, this, "registry version=%d", arg.version);
  ...
  if(0 < encoded_len)
  {
    ICM4X6XX_VERB_PRINTF(LOW, this, "registry reg_data_stream=%x", (uint32_t)state->reg_data_stream);
...
#define ICM40607A_WHOAMI_VALUE          0x45
...
#define ICM42631_WHOAMI_VALUE           0x5C
#define ICM42632_WHOAMI_VALUE           0x6A
...
static bool icm4x6xx_check_product_info(sns_sensor *this, uint8_t whoami)
{
    ...
    ICM4X6XX_PRINTF(MED, this, "WHO AM I IS 0x%x", whoami);
    ...
    case ICM42622_WHOAMI_VALUE:
        ...
        state->common.product_info.series = YOKOHAMA;
        break;
    case ICM42631_WHOAMI_VALUE:
        hw_is_present = true;
        state->common.product_info.series = YOKO_C1;
        break;
    case ICM42632_WHOAMI_VALUE:
        hw_is_present = true;
        state->common.product_info.series = YOKO_C1;
        break;
    ...
    case ICM42688_WHOAMI_VALUE:
        hw_is_present = true;
        /* High resolution (i.e. 20-bits data format) is applicable only
           when accel FSR is +/-16G and gyro FSR is +/-2000dps */
    ...
    if (YOKOHAMA == state->common.product_info.series) {
        ...
        if (ICM40607_WHOAMI_VALUE == whoami) {
            state->common.product_info.i3c_supported = false;
        } else if (ICM40607A_WHOAMI_VALUE == whoami) {
            state->common.product_info.i3c_supported = false;
        } else {
    ...
    } else if (YOKO_C1 == state->common.product_info.series) {
        state->common.product_info.i3c_supported = true;
        state->common.product_info.fifo_size =
            ICM4X6XX_YOKOHAMA_MAX_FIFO * FIFO_USE_PROPORTION;
...
bool icm4x6xx_discover_hw(sns_sensor *const this)
{
    ...
    if (hw_is_present) {
        ...
        struct devinfo info_acc = {
            .cal_path = ICM4X6XX_GEN_GROUP(0, PLATFORM_FAC_CAL_ACCEL) "" ICM4X6XX_FAC_CAL_BIAS,
        };
        struct devinfo info_gyro = {
            .cal_path = ICM4X6XX_GEN_GROUP(0, PLATFORM_FAC_CAL_GYRO) "" ICM4X6XX_FAC_CAL_BIAS,
        };
        ...
        if (!register_deinfo) {
            ...
            SNS_PRINTF(HIGH, this, "icm4x6xx gsensor and gyro register_sensor_devinfo");
        }
    }
    return hw_is_present;
}
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
        /* For HW pedometer, the minim ODR should be big than 50Hz */
        sns_std_attr_value_data values[] = {SNS_ATTR, SNS_ATTR, SNS_ATTR, SNS_ATTR};
        values[0].has_flt = true;
        values[0].flt = ICM4X6XX_ODR_50 * odr_adjust_rate;
        values[1].has_flt = true;
        values[1].flt = ICM4X6XX_ODR_100 * odr_adjust_rate;
        values[2].has_flt = true;
        values[2].flt = ICM4X6XX_ODR_200 * odr_adjust_rate;
        values[3].has_flt = true;
        values[3].flt = ICM4X6XX_ODR_500 * odr_adjust_rate;
#else
        sns_std_attr_value_data values[] = {SNS_ATTR, SNS_ATTR, SNS_ATTR, SNS_ATTR,
...
static void icm4x6xx_update_suid_to_accel_sensors(sns_sensor *const this)
{
    ...
    for (sensor = this->cb->get_library_sensor(this, true);
        ...
        if (state->sensor == ICM4X6XX_GYRO) {
            ...
            acc_state->common.step_counter_suid = state->my_suid;
#endif
        }
    }
}
