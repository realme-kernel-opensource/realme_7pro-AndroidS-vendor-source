...
#define DMP_START_UP_TIME (50 * 1000 * 1000) //50ms, unit is ns
#endif
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
#define GYRO_RING_DOWN_TIME (100 * 1000 * 1000) //100ms, unit is ns
#endif
...
typedef struct icm4x6xx_fifo_info
{
    ...
    /** current FIFO Data Rate, fdr = odr/fdr_factor */
    uint8_t                 cur_fdr_factor;
    ...
    uint64_t               prev_max_requested_flush_ticks;
#endif
...
typedef struct icm4x6xx_accel_info
{
  ...
  icm4x6xx_cfg_evt_info    last_sent_cfg_p;
#endif
...
typedef struct icm4x6xx_gyro_info
{
  ...
  bool                     actual_publish;
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
  /** need to postpone enable gyro */
  bool                     postpone_enable;
...
#endif /*ICM4X6XX_REMOVE_FILTER_DELAY*/
#ifdef ICM4X6XX_GYRO_RING_DOWN_WA
  /** ts of gyro last turned off(tick) */
  sns_time                 last_off_tick;
  sns_time                 postpone_tick;
  sns_data_stream          *timer_postpone_stream;
#endif /*ICM4X6XX_GYRO_RING_DOWN_WA*/
} icm4x6xx_gyro_info;
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
typedef struct icm4x6xx_step_counter_info
{
  sns_sensor_uid          suid;
  bool                    is_enabled;
  bool                    desire_en;
  bool                    desire_publish;
  bool                    is_client_present:1;
  bool                    is_new_req;
  bool                    is_timer_active;

  float                   cur_sampling_rate_hz;
  float                   sampling_rate_hz;
  int16_t                 a_sample_value[25];
  uint16_t                cnt;
  uint16_t                step_counter_cnt;
  uint16_t                client_cnt;
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
  bool                      en_pedometer;
  bool                      desire_en_pedometer;
  bool                      desire_pub_pedometer;
#endif
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
  /** step counter info */
  icm4x6xx_step_counter_info  step_counter_info;
#endif
...
#endif
#ifdef ICM4X6XX_PASSIVE_SENSOR_SUPPORT
  icm4x6xx_sensor_type      passive_client_present;
#endif
...
#ifdef ICM4X6XX_ENABLE_PEDOMETER
  size_t                    encoded_sensor_pedometer_event_len;
#endif
...
/**
 * Send out error event to framework
 *
 * @param[i] this          Sensor instance
 * @param[i] sensor_type   Sensor type
 * @param[i] sns_std_error Error type
 */
