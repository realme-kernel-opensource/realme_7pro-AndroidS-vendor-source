/**
 * @file sns_mmc5603x_magnetic_sensor.c
 *
 * MMC5603X Magnetic virtual Sensor implementation.
 *
...
#ifndef MMC5603X_ENABLE_ALL_DEVICES
 ...
 * within notify_event.
 */
static void mmc5603x_publish_default_attributes(sns_sensor *const this)
{
  ...
  }
  {
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
    value.has_sint = true;
    value.sint = MMC5603X_DRIVER_VERSION;
    sns_publish_attribute(
...
sns_rc mmc5603x_mag_init(sns_sensor *const this)
{
  ...
  mmc5603x_publish_default_attributes(this);
  ...
  MMC5603X_PRINT(ERROR, this, "registration_idx=%d",state->registration_idx);
#endif

#ifdef MMC5603X_ENABLE_ALL_DEVICES
  uint8_t i = 0;
...
#else
  MMC5603X_PRINT(ERROR, this, "SEE-Lite Mode.");

#ifdef MMC5603X_STATE_IS_DRI
  state->is_dri = true;
#else
  state->is_dri = false;
#endif

#ifdef MMC5603X_STATE_USE_FIFO
  state->use_fifo = true;
#else
  state->use_fifo = false;
#endif

#ifdef MMC5603X_STATE_SUPPORTS_SYNC_STREAM
  ...
  state->com_port_info.com_config.bus_instance = 1;       // DragonBoard
  state->com_port_info.com_config.slave_control = 0x30;     // Slave address:0x0C
...
#endif // MMC5603X_ENABLE_ALL_DEVICES
