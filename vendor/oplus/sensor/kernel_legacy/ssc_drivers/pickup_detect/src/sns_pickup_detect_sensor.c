...
static void pickup_detect_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "pick_up_motion";
    ...
    {
        char const type[] = "pick_up_motion";
        sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
...
static void pickup_detect_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    PICKUP_DETECT_LOG_0("pickup_detect_publish_available");
...
sns_rc sns_pickup_detect_init(sns_sensor *const this)
{
    sns_pickup_detect_sensor_state *state = (sns_pickup_detect_sensor_state *)this->state->state;
    struct sns_service_manager *smgr = this->cb->get_service_manager(this);
    float data[2] = {0};

    oppo_get_virtual_sensor(OPPO_PICKUP_DETECT, &state->config.smem);
    ...
    pickup_detect_publish_attributes(this);
    PICKUP_DETECT_LOG_0("pickup_detect init success and attributes published");

    return SNS_RC_SUCCESS;
}

sns_rc sns_pickup_detect_deinit(sns_sensor *const this)
{
    UNUSED_VAR(this);
    PICKUP_DETECT_LOG_0("sns_pickup_detect_deinit");
    return SNS_RC_SUCCESS;
}

sns_rc sns_pickup_detect_notify_event(sns_sensor *const this)
{
    sns_pickup_detect_sensor_state *state = (sns_pickup_detect_sensor_state *)this->state->state;

    PICKUP_DETECT_LOG_0("sns_pickup_detect_notify_event");

    sns_suid_lookup_handle(this, &state->suid_lookup_data);

    if (sns_suid_lookup_complete(&state->suid_lookup_data)) {
        pickup_detect_publish_available(this);
...
sns_sensor_instance *sns_pickup_detect_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    PICKUP_DETECT_LOG_2("pickup_detect_set_client_request: exist_request = 0x%x, new_request = 0x%x",
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        PICKUP_DETECT_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            PICKUP_DETECT_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_pickup_detect_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            PICKUP_DETECT_LOG_0("set_client_config");
    ...
    if (NULL != ret_inst
        && NULL == ret_inst->cb->get_client_request(ret_inst, this->sensor_api->get_sensor_uid(this),
            true)) {
        PICKUP_DETECT_LOG_0("remove_instance");
