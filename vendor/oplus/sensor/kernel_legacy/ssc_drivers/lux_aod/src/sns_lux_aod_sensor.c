...
static void lux_aod_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "lux_aod";
...
static void lux_aod_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    LUX_AOD_LOG_0("lux_aod_publish_available");
...
{
    sns_lux_aod_sensor_state *state = (sns_lux_aod_sensor_state *)this->state->state;
    struct sns_service_manager *smgr = this->cb->get_service_manager(this);
    float data[2] = {0};

    oppo_get_virtual_sensor(OPPO_LUX_AOD, &state->config.smem);
    ...
    if (0 == state->config.smem->parameter[2]) {
        sns_suid_lookup_add(this, &state->suid_lookup_data, "ambient_light");
    } else {
        sns_suid_lookup_add(this, &state->suid_lookup_data, "wise_light");
    }

    lux_aod_publish_attributes(this);
    LUX_AOD_LOG_0("lux_aod init success and attributes published");
...
{
    UNUSED_VAR(this);
    LUX_AOD_LOG_0("sns_lux_aod_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_sensor_instance *sns_lux_aod_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    LUX_AOD_LOG_2("lux_aod_set_client_request: exist_request = 0x%x, new_request = 0x%x", exist_request,
        new_request);
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        LUX_AOD_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            LUX_AOD_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_lux_aod_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            LUX_AOD_LOG_0("set_client_config");
