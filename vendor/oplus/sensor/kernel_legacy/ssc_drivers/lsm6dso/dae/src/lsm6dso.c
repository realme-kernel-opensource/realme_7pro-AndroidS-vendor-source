...
*****************************************************************************************
*/
static uint8_t lsm6dso_wake_src = 0;

/* ------------------------------------------------------------------------------------ */
static sns_com_port_status_e
lsm6dso_get_data( sns_dd_handle_s*    dd_handle,
                  ...
                  int32_t*            num_samples )
{
  ...
  if( status == SNS_COM_PORT_STATUS_SUCCESS && wake_src != 0xff )
  {
    ...
    {
      { .reg_addr = STM_LSM6DSO_REG_CTRL1_XL,
        .buf_sz   = 2 },
      { .reg_addr = STM_LSM6DSO_REG_OUT_X_L_G,
        .buf_sz   = 12 },
    };
    *num_samples = 1;
...
/* ------------------------------------------------------------------------------------ */
static sns_com_port_status_e
lsm6dso_parse_accel_data( sns_dd_handle_s*        dd_handle,
...
sns_dd_if_s lsm6dso_hal_table =
  { .get_data = lsm6dso_get_data,
    .parse_accel_data = lsm6dso_parse_accel_data };
