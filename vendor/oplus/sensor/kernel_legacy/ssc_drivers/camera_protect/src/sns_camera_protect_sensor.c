...
static void camera_protect_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "camera_protect_detect";
    ...
    {
        char const type[] = "camera_protect_detect";
        sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
...
static void camera_protect_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    CAMERA_PROTECT_LOG_0("camera_protect_publish_available");
...
{
    sns_camera_protect_sensor_state *state = (sns_camera_protect_sensor_state *)this->state->state;
    struct sns_service_manager *smgr = this->cb->get_service_manager(this);
    float data[2] = {0};

    oppo_get_virtual_sensor(OPPO_CAMERA_PROTECT, &state->config.smem);
    ...
    camera_protect_publish_attributes(this);
    CAMERA_PROTECT_LOG_0("camera_protect init success and attributes published");
...
{
    UNUSED_VAR(this);
    CAMERA_PROTECT_LOG_0("camera_protect_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_sensor_instance *sns_camera_protect_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    CAMERA_PROTECT_LOG_2("camera_protect_set_client_request: exist_request = 0x%x, new_request = 0x%x",
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        CAMERA_PROTECT_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            CAMERA_PROTECT_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_camera_protect_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            CAMERA_PROTECT_LOG_0("set_client_config");
    ...
    if (NULL != ret_inst
        && NULL == ret_inst->cb->get_client_request(ret_inst, this->sensor_api->get_sensor_uid(this),
            true)) {
        CAMERA_PROTECT_LOG_0("remove_instance");
