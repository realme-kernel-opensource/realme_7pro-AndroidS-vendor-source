...
** File: - sns_oplus_activity_recognition_sensor.c
...
static void oplus_activity_recognition_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "oplus_activity_recognition";
    ...
    {
        char const type[] = "oplus_activity_recognition";
        sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
...
static void oplus_activity_recognition_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    OPLUS_ACTIVITY_RECOGNITION_LOG_0("oplus_activity_recognition_publish_available");
...
{
    sns_oplus_activity_recognition_sensor_state *state = (sns_oplus_activity_recognition_sensor_state *) this->state->state;
    ...
    state->config.encoded_data_event_len = pb_get_encoded_size_sensor_stream_event(data, 5);
    ...
    oplus_activity_recognition_publish_attributes(this);
    OPLUS_ACTIVITY_RECOGNITION_LOG_0("oplus_activity_recognition init success and attributes published");
...
{
    UNUSED_VAR(this);
    OPLUS_ACTIVITY_RECOGNITION_LOG_0("sns_oplus_activity_recognition_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_rc sns_oplus_activity_recognition_notify_event(sns_sensor *const this)
{
    ...
    OPLUS_ACTIVITY_RECOGNITION_LOG_0("sns_oplus_activity_recognition_notify_event");

    sns_suid_lookup_handle(this, &state->suid_lookup_data);

    if (sns_suid_lookup_complete(&state->suid_lookup_data)) {
        oplus_activity_recognition_publish_available(this);
...
sns_sensor_instance *sns_oplus_activity_recognition_set_client_request(sns_sensor *const this, sns_request const *exist_request,
                                                     sns_request const *new_request, bool remove)
{
    sns_sensor_instance *ret_inst = sns_sensor_util_get_shared_instance(this);

    OPLUS_ACTIVITY_RECOGNITION_LOG_2("oplus_activity_recognition_set_client_request: exist_request = 0x%x, new_request = 0x%x", exist_request, new_request);
    ...
    if (remove) {
        if (NULL != ret_inst) {
            ret_inst->cb->remove_client_request(ret_inst, exist_request);
                ...
                (NULL != ret_inst)) {
        OPLUS_ACTIVITY_RECOGNITION_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            OPLUS_ACTIVITY_RECOGNITION_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_oplus_activity_recognition_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            OPLUS_ACTIVITY_RECOGNITION_LOG_0("set_client_config");
    ...
    if (NULL != ret_inst &&
        ...
        OPLUS_ACTIVITY_RECOGNITION_LOG_0("remove_instance");
