...
static void near_ear_detect_publish_attributes(sns_sensor *const this)
{
    {
        char const name[] = "near_ear_detect";
    ...
    {
        char const type[] = "near_ear_detect";
        sns_std_attr_value_data value = sns_std_attr_value_data_init_default;
...
static void near_ear_detect_publish_available(sns_sensor *const this)
{
    sns_std_attr_value_data value = sns_std_attr_value_data_init_default;

    NEAR_EAR_DETECT_LOG_0("near_ear_detect_publish_available");
...
{
    sns_near_ear_detect_sensor_state *state = (sns_near_ear_detect_sensor_state *)this->state->state;
    ...
    near_ear_detect_publish_attributes(this);
    NEAR_EAR_DETECT_LOG_0("near_ear_detect init success and attributes published");
...
{
    UNUSED_VAR(this);
    NEAR_EAR_DETECT_LOG_0("sns_near_ear_detect_deinit");
    return SNS_RC_SUCCESS;
}
...
sns_sensor_instance *sns_near_ear_detect_set_client_request(sns_sensor *const this,
    sns_request const *exist_request, sns_request const *new_request, bool remove)
{
    ...
    NEAR_EAR_DETECT_LOG_2("near_ear_detect_set_client_request: exist_request = 0x%x, new_request = 0x%x",
    ...
    if (remove) {
        ...
        && (NULL != ret_inst)) {
        NEAR_EAR_DETECT_LOG_0("Flush client req");
        ...
        if (NULL == ret_inst) {
            ...
            NEAR_EAR_DETECT_LOG_0("create_instance");

            ret_inst = this->cb->create_instance(this, (sizeof(sns_near_ear_detect_inst_state)));
        }

        if ((NULL != ret_inst) && (NULL == exist_request)) {
            NEAR_EAR_DETECT_LOG_0("set_client_config");
    ...
    if (NULL != ret_inst
        && NULL == ret_inst->cb->get_client_request(ret_inst, this->sensor_api->get_sensor_uid(this),
            true)) {
        NEAR_EAR_DETECT_LOG_0("remove_instance");
