...
#define NEAR_EAR_DETECT_BATCH_PERIOD 100000  // 100ms

sns_sensor_instance *near_ear_detect_mTask;

static void near_ear_detect_sensor_enable(int sensor_type, bool on)
{
    sns_near_ear_detect_inst_state *inst_state = (sns_near_ear_detect_inst_state *)
        near_ear_detect_mTask->state->state;
    sns_rc rc = SNS_RC_SUCCESS;

    float sample_rate = inst_state->state->sampleRate;

    NEAR_EAR_DETECT_LOG_2("sensor enable operate, sensor_type = %d, on = %d", sensor_type, on);

    sns_service_manager *manager = near_ear_detect_mTask->cb->get_service_manager(near_ear_detect_mTask);
    ...
    std_req.batching.batch_period = NEAR_EAR_DETECT_BATCH_PERIOD;
    ...
    if (on) {
        ...
        case ACC_TYPE:
            ...
            if (NULL == inst_state->resampler_accel_stream) {
                NEAR_EAR_DETECT_LOG_0("create resampler acc stream.");

                // create connection with resampler sensor
                stream_service->api->create_sensor_instance_stream(stream_service, near_ear_detect_mTask,
            ...
            if (NULL != inst_state->resampler_accel_stream) {
                ...
                if (0 < encoded_len) {
                    ...
                    rc = inst_state->resampler_accel_stream->api->send_request(inst_state->resampler_accel_stream,
                            &request);
                    NEAR_EAR_DETECT_LOG_2("Processed accel config request: sample_rate %f, result %u", sample_rate, rc);
                }
            } else {
                NEAR_EAR_DETECT_LOG_0("Error in creating accel_stream OR encoding failed");
            }
            break;
        ...
        case GYRO_TYPE:
            ...
            if (NULL == inst_state->resampler_gyro_stream) {
                NEAR_EAR_DETECT_LOG_0("create resampler gyro stream.");
                // create connection with resampler sensor
                stream_service->api->create_sensor_instance_stream(stream_service, near_ear_detect_mTask,
            ...
            if (NULL != inst_state->resampler_gyro_stream) {
                ...
                if (0 < encoded_len) {
                    ...
                    rc = inst_state->resampler_gyro_stream->api->send_request(inst_state->resampler_gyro_stream,
                            &request);
                    NEAR_EAR_DETECT_LOG_2("Processed gyro config request: sample_rate %f, result %u", sample_rate, rc);
                }
            } else {
                NEAR_EAR_DETECT_LOG_0("Error in creating gyro_stream OR encoding failed");
            }
        ...
        case GRAVITY_TYPE:
            ...
            if (NULL == inst_state->resampler_gravity_stream) {
                NEAR_EAR_DETECT_LOG_0("create gravity stream.");
            ...
            if (NULL != inst_state->resampler_gravity_stream) {
                ...
                if (0 < encoded_len) {
                    ...
                    rc = inst_state->resampler_gravity_stream->api->send_request(inst_state->resampler_gravity_stream,
                            &request);
                    NEAR_EAR_DETECT_LOG_2("Processed gravity config request: sample_rate %f, result %u", sample_rate, rc);
                }
            } else {
                NEAR_EAR_DETECT_LOG_0("Error in creating gravity_stream OR encoding failed");
            }
        ...
        case PROX_TYPE:
            ...
            if (NULL == inst_state->resampler_prox_stream) {
                NEAR_EAR_DETECT_LOG_0("create prox stream.");
            ...
            if (NULL != inst_state->resampler_prox_stream) {
                ...
                if (0 < encoded_len) {
                    ...
                    rc = inst_state->resampler_prox_stream->api->send_request(inst_state->resampler_prox_stream,
                            &request);
                    NEAR_EAR_DETECT_LOG_2("Processed prox config request: sample_rate %f, result %u", sample_rate, rc);
                }
            } else {
                NEAR_EAR_DETECT_LOG_0("Error in creating prox_stream OR encoding failed");
            }
        ...
        switch (sensor_type) {
        case ACC_TYPE:
            sns_sensor_util_remove_sensor_instance_stream(near_ear_detect_mTask,
...
static void near_ear_detect_report(int value, uint16_t report_count)
{
    sns_rc rc = SNS_RC_SUCCESS;
    float near_ear_detect_data[2] = {0};
    sns_near_ear_detect_inst_state *inst_state = (sns_near_ear_detect_inst_state *)
        near_ear_detect_mTask->state->state;

    near_ear_detect_data[0] = value;
    near_ear_detect_data[1] = (float)report_count;

    rc = pb_send_sensor_stream_event(near_ear_detect_mTask,
            NULL,
            sns_get_system_time(),
            SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT,
            SNS_STD_SENSOR_SAMPLE_STATUS_ACCURACY_HIGH,
            near_ear_detect_data,
            2,
            inst_state->config.encoded_data_event_len);
    if (SNS_RC_SUCCESS != rc) {
        NEAR_EAR_DETECT_LOG_0("free fall report failed, Error in sending event");
    } else {
        NEAR_EAR_DETECT_LOG_0("report near_ear_detect data.");
    }
}

static uint64_t near_ear_detect_get_delta_time_ms(uint64_t timestamp)
{
    return  sns_get_ms_time_from_tick(timestamp);
}

static bool near_ear_detect_need_check_prox(void)
{
    return true;
}

static void near_ear_detect_decode_and_checkdata(sns_sensor_event *event_in, SENSOR_TYPE type_in)
{
    uint8_t arr_index = 0;
    float data[6] = {0};
    near_ear_detect_sensor_data input;
    bool need_handle = false;

    if (SNS_STD_SENSOR_MSGID_SNS_STD_SENSOR_EVENT == event_in->message_id) {
        NEAR_EAR_DETECT_LOG_3("earliest input_type = %d, message_id = %d, timestamp = %llu", type_in,
        ...
        if (!pb_decode(&istream, sns_std_sensor_event_fields, &decoded_event)) {
            NEAR_EAR_DETECT_LOG_0("Error in decoding event");
        } else {
            need_handle = true;
            NEAR_EAR_DETECT_LOG_3("data[%d %d %d]", (int)(1000 * data[0]), (int)(1000 * data[1]),
                (int)(1000 * data[2]));
        }
        ...
        if (!pb_decode(&istream, sns_proximity_event_fields, &decoded_event)) {
            NEAR_EAR_DETECT_LOG_0("Error in decoding prox event");
        } else {
            need_handle = true;
    ...
    if (true == need_handle) {
        ...
        near_ear_detect_process_sensor_data(&input);
    }
}

static sns_rc sns_near_ear_detect_process_event(sns_sensor_instance *const this)
{
    ...
    if (NULL != inst_state->resampler_accel_stream) {
        ...
        while (NULL != acc_event) {
            near_ear_detect_decode_and_checkdata(acc_event, ACC_TYPE);

            // stream maybe released in near_ear_detect_decode_and_checkdata(), so must judge this stream
            if (NULL != inst_state->resampler_accel_stream) {
                ...
                NEAR_EAR_DETECT_LOG_0("accel stream has been released.");
                acc_event = NULL;
            }
        }
    }
    ...
    if (NULL != inst_state->resampler_gyro_stream) {
        ...
        while (NULL != gyro_event) {
            near_ear_detect_decode_and_checkdata(gyro_event, GYRO_TYPE);

            if (NULL != inst_state->resampler_gyro_stream) {
                gyro_event = inst_state->resampler_gyro_stream->api->get_next_input(
                        inst_state->resampler_gyro_stream);
            } else {
                NEAR_EAR_DETECT_LOG_0("gyro stream has been released.");
                gyro_event = NULL;
            }
        }
    }
    ...
    if (NULL != inst_state->resampler_gravity_stream) {
        ...
        while (NULL != gravity_event) {
            near_ear_detect_decode_and_checkdata(gravity_event, GRAVITY_TYPE);

            if (NULL != inst_state->resampler_gravity_stream) {
                gravity_event = inst_state->resampler_gravity_stream->api->get_next_input(
                        inst_state->resampler_gravity_stream);
            } else {
                NEAR_EAR_DETECT_LOG_0("gravity stream has been released.");
                gravity_event = NULL;
            }
        }
    }
    ...
    if (NULL != inst_state->resampler_prox_stream) {
        ...
        while (NULL != prox_event) {
            near_ear_detect_decode_and_checkdata(prox_event, PROX_TYPE);

            if (NULL != inst_state->resampler_prox_stream) {
                prox_event = inst_state->resampler_prox_stream->api->get_next_input(
                        inst_state->resampler_prox_stream);
            } else {
                NEAR_EAR_DETECT_LOG_0("prox stream has been released.");
                prox_event = NULL;
            }
        }
    }
...
static sns_rc sns_near_ear_detect_inst_notify_event(sns_sensor_instance *const this)
{
    sns_rc rc = SNS_RC_SUCCESS;

    sns_near_ear_detect_process_event(this);

    return rc;
}

sns_sensor_instance_api sns_near_ear_detect_sensor_instance_api = {
    .struct_len = sizeof(sns_sensor_instance_api),
    .init = &sns_near_ear_detect_inst_init,
    .deinit = &sns_near_ear_detect_inst_deinit,
    .set_client_config = &sns_near_ear_detect_inst_set_client_config,
    .notify_event = &sns_near_ear_detect_inst_notify_event
};

struct near_ear_detect_sensor_operation near_ear_detect_ops = {
    .sensor_change_state = near_ear_detect_sensor_enable,
    .report = near_ear_detect_report,
    .get_delta_time_ms = near_ear_detect_get_delta_time_ms,
    .need_check_prox = near_ear_detect_need_check_prox,
};
