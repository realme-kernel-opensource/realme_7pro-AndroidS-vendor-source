...
static int zy0603_disable_ssdf_and_pf(struct chip_bq27541 *chip)
{
	...
	if (!zy0603_unseal()) {	//ok
		...
		if(!((read_buf[0]==write_static_df_cheksum[0])&&(read_buf[1]==write_static_df_cheksum[1])&&(read_buf[2]==write_static_df_cheksum[2])&&(read_buf[3]==write_static_df_cheksum[3])))
		{
			rc = -1;
			return rc;
		}
